...
fn context_returns_source_callers_imports_and_conformance() -> Result<()> {
    ...
    assert!(
        result
            .source
            .contains("export class Cache implements Reader")
    );
    ...
    assert!(
        result
            .callers
            .iter()
            .any(|reference| reference.rel_path == "src/main.ts")
    );
...
fn investigate_adapts_for_functions_and_types() -> Result<()> {
    ...
    assert!(
        function
            .refs
            .iter()
            .any(|reference| reference.rel_path == "src/main.ts" && reference.line > 0)
    );
