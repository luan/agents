...
const EXIT_SENTINEL: &str = "__code_mode_runtime_exit__";
...
fn run_runtime(
    ...
    let isolate =
        &mut v8::Isolate::new(v8::CreateParams::default().heap_limits(0, 128 * 1024 * 1024));
    ...
    while let Some(command) = next_runtime_command(
        &event_tx,
        &command_rx,
        &control_rx,
        initial_pending_frontier,
...
fn next_runtime_command(
    ...
    let mut pending_frontier_reported = !initial_pending_frontier;
    loop {
        if !pending_frontier_reported {
            pending_frontier_reported = true;
