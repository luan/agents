...
const MAX_STORED_KEYS: usize = 128;
const MAX_STORED_BYTES: usize = 8 * 1024 * 1024;
const OUTPUT_TRUNCATED_TEXT: &str = "[Code Mode output truncated by the runtime]";
...
fn emit_content_item(state: &mut RuntimeState, item: FunctionCallOutputContentItem) {
    let bytes = output_item_bytes(&item);
    if state.output_items >= MAX_OUTPUT_ITEMS
        || state.output_bytes.saturating_add(bytes) > MAX_OUTPUT_BYTES
    {
        if !state.output_truncated {
            state.output_truncated = true;
            let _ = state.event_tx.send(RuntimeEvent::ContentItem(
                FunctionCallOutputContentItem::InputText {
                    text: OUTPUT_TRUNCATED_TEXT.to_string(),
                },
            ));
        }
        return;
    }
    state.output_items += 1;
...
pub(super) fn tool_callback(
    ...
    if scope
        ...
        .is_some_and(|state| state.pending_tool_calls.len() >= MAX_PENDING_TOOL_CALLS)
    {
        throw_type_error(scope, "too many pending nested tool calls");
        return;
    }
...
pub(super) fn text_callback(
    ...
    if let Some(state) = scope.get_slot_mut::<RuntimeState>() {
        emit_content_item(state, FunctionCallOutputContentItem::InputText { text });
    }
...
pub(super) fn audio_callback(
    ...
    if let Some(state) = scope.get_slot_mut::<RuntimeState>() {
        emit_content_item(state, audio_item);
    }
    retval.set(v8::undefined(scope).into());
}
...
pub(super) fn image_callback(
    ...
    if let Some(state) = scope.get_slot_mut::<RuntimeState>() {
        emit_content_item(state, image_item);
    }
    retval.set(v8::undefined(scope).into());
}
...
pub(super) fn store_callback(
    ...
    let violation = scope.get_slot::<RuntimeState>().and_then(|state| {
        let replacing = state.stored_values.contains_key(&key);
        if !replacing && state.stored_values.len() >= MAX_STORED_KEYS {
            return Some("stored values exceed the 128-key limit");
        }
        ...
        let current_bytes = state
            ...
            .map(|bytes| bytes.len())
            .sum::<usize>();
        let next_bytes = serde_json::to_vec(&serialized).map_or(usize::MAX, |bytes| bytes.len());
        if current_bytes
            .saturating_sub(previous_bytes)
            .saturating_add(next_bytes)
            > MAX_STORED_BYTES
        {
            return Some("stored values exceed the 8 MiB limit");
        }
        None
    });
