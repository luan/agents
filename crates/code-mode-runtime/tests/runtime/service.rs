...
use code_mode_runtime::CellId;
use code_mode_runtime::CodeModeNestedToolCall;
use code_mode_runtime::CodeModeSessionDelegate;
use code_mode_runtime::CodeModeToolKind;
use code_mode_runtime::ExecuteRequest;
use code_mode_runtime::ExecuteToPendingOutcome;
use code_mode_runtime::FunctionCallOutputContentItem;
use code_mode_runtime::InProcessCodeModeSession;
use code_mode_runtime::NotificationFuture;
use code_mode_runtime::RuntimeResponse;
use code_mode_runtime::ToolDefinition;
use code_mode_runtime::ToolInvocationFuture;
use code_mode_runtime::WaitOutcome;
use code_mode_runtime::WaitRequest;
use code_mode_runtime::WaitToPendingOutcome;
use code_mode_runtime::WaitToPendingRequest;
use code_mode_runtime::yield_timeout;
...
struct YieldTime(#[proptest(strategy = "0u64..=1_000_000")] u64);

proptest! {
    #![proptest_config(ProptestConfig::with_cases(16))]

    // Property: applying the grace policy never shortens the caller's timeout.
    #[test]
    fn yield_timeout_never_shortens_the_requested_duration(input: YieldTime) {
        let requested = Duration::from_millis(input.0);
        prop_assert!(yield_timeout(input.0) >= requested);
    }
}
...
impl CodeModeSessionDelegate for ReleasableToolDelegate {
    ...
    ) -> ToolInvocationFuture<'_> {
        self.tool_started.store(true, Ordering::Release);
        Box::pin(async move {
            tokio::select! {
                () = self.tool_release.notified() => Ok(JsonValue::Null),
                () = cancellation_token.cancelled() => Err("cancelled".to_string()),
            }
        })
    }
...
#[tokio::test]
async fn stored_values_reject_excess_keys_and_bytes() {
    let key_session = InProcessCodeModeSession::new();
    let key_response = execute(
        &key_session,
        ExecuteRequest {
            source: r"for (let i = 0; i < 129; i++) store(`key-${i}`, i);".to_string(),
            yield_time_ms: None,
            ..execute_request("")
        },
    )
    .await;
    let RuntimeResponse::Result { error_text, .. } = key_response else {
        panic!("expected key-limit result");
    };
    assert!(error_text.is_some_and(|text| text.contains("128-key limit")));

    let byte_session = InProcessCodeModeSession::new();
    let byte_response = execute(
        &byte_session,
        ExecuteRequest {
            source: r#"store("large", "x".repeat(9 * 1024 * 1024));"#.to_string(),
            yield_time_ms: None,
            ..execute_request("")
        },
    )
    .await;
    let RuntimeResponse::Result { error_text, .. } = byte_response else {
        panic!("expected byte-limit result");
    };
    assert!(error_text.is_some_and(|text| text.contains("8 MiB limit")));
}

#[tokio::test]
async fn one_cell_rejects_excess_pending_nested_calls() {
    let service =
        InProcessCodeModeSession::with_delegate(Arc::new(ReleasableToolDelegate::default()));
    let response = execute(
        &service,
        ExecuteRequest {
            enabled_tools: vec![echo_tool()],
            source: r"for (let i = 0; i < 257; i++) tools.echo({ index: i });".to_string(),
            yield_time_ms: None,
            ..execute_request("")
        },
    )
    ...
    let RuntimeResponse::Result { error_text, .. } = response else {
        panic!("expected nested-call limit result");
    };
    assert!(error_text.is_some_and(|text| text.contains("too many pending nested tool calls")));
}
        ...
        RuntimeResponse::Result {
            ...
            content_items: vec![
                ...
                FunctionCallOutputContentItem::InputImage {
                    ...
                    detail: Some(code_mode_runtime::DEFAULT_IMAGE_DETAIL),
                },
    ...
    assert_eq!(
            ...
            RuntimeResponse::Result {
                ...
                content_items: vec![FunctionCallOutputContentItem::InputImage {
                    ...
                    detail: Some(code_mode_runtime::ImageDetail::Original),
                }],
                error_text: None,
            }
        );
...
"#
    ...
    assert_eq!(
            ...
            RuntimeResponse::Result {
                ...
                content_items: vec![FunctionCallOutputContentItem::InputImage {
                    image_url: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGP4z8DwHwAFAAH/iZk9HQAAAABJRU5ErkJggg==".to_string(),
                    detail: Some(code_mode_runtime::ImageDetail::High),
                }],
                error_text: None,
            }
        );
...
"#
    ...
    assert_eq!(
        ...
        RuntimeResponse::Result {
            ...
            content_items: vec![FunctionCallOutputContentItem::InputImage {
                image_url: "data:image/png;base64,AAA".to_string(),
                detail: Some(code_mode_runtime::ImageDetail::Low),
            }],
            error_text: None,
        }
    );
