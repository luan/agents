...
impl HostPeer {
    ...
    pub(super) fn respond(
        &self,
        id: RequestId,
        result: Result<code_mode_protocol::host::HostResponse, String>,
    ) {
    ...
    fn initial_response(
        &self,
        id: RequestId,
        result: Result<code_mode_protocol::host::WireRuntimeResponse, String>,
    ) {
    ...
    pub(super) fn start_cell(
        ...
        self.spawn_critical("cell forwarding", async move {
            drive_cell(
                ...
                (active_cell_permit, cancellation, session),
            )
            .await;
        });
...
async fn drive_cell(
    ...
    let (_active_cell_permit, cancellation, session) = lifetime;
    ...
    let closed = loop {
        tokio::select! {
            ...
            _ = cancellation.cancelled() => {
                let _ = session.terminate(key.1.clone()).await;
