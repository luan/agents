// Keep the pinned upstream host protocol shapes recognizable.
#![allow(
...
use code_mode_protocol::host::EncodedFrame;
        ...
        match request {
            ...
            HostRequest::Execute {
                ...
                match result {
                    Ok(started) => {
                        if cancellation.is_cancelled() {
                            let _ = session.terminate(started.cell_id.clone()).await;
                            self.respond(
                                request_id,
...
struct RequestRegistry {
    ...
    recent: HashMap<RequestId, ActiveRequest>,
...
impl RequestRegistry {
    fn start(
        ...
        if self.active.contains_key(&request_id) || self.recent.contains_key(&request_id) {
    ...
    fn cancel(&self, request_id: RequestId) {
        if let Some(request) = self
            .active
            .get(&request_id)
            .or_else(|| self.recent.get(&request_id))
    ...
    fn finish(&mut self, request_id: RequestId) {
        let Some(request) = self.active.remove(&request_id) else {
            return;
        };
        self.recent.insert(request_id, request);
