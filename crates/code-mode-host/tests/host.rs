...
use code_mode_protocol::host::FramedReader;
use code_mode_protocol::host::FramedWriter;
...
struct ProtocolVersionInput(u32);

proptest! {
    #![proptest_config(ProptestConfig::with_cases(16))]

    // Property: every nonzero protocol version survives its wire representation unchanged.
    #[test]
    fn protocol_versions_round_trip_on_the_wire(input: ProtocolVersionInput) {
        prop_assume!(input.0 > 0);
        let version = ProtocolVersion::new(input.0).expect("nonzero protocol version");
        let encoded = serde_json::to_value(version).expect("serialize protocol version");
        assert_eq!(
            serde_json::from_value::<ProtocolVersion>(encoded).expect("deserialize protocol version"),
            version
        );
    }
}
