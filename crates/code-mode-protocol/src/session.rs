...
pub trait CodeModeSession: Send + Sync {
    fn execute(&self, request: ExecuteRequest) -> CodeModeSessionResultFuture<'_, StartedCell>;

    fn wait(&self, request: WaitRequest) -> CodeModeSessionResultFuture<'_, WaitOutcome>;

    fn terminate(&self, cell_id: CellId) -> CodeModeSessionResultFuture<'_, WaitOutcome>;

    fn shutdown(&self) -> CodeModeSessionResultFuture<'_, ()>;
}
...
pub trait CodeModeSessionProvider: Send + Sync {
    ...
    ) -> CodeModeSessionProviderFuture<'_>;
}
