...
pub fn share_root() -> Result<PathBuf, Box<dyn std::error::Error>> {
...
pub fn cache_root() -> Result<PathBuf, Box<dyn std::error::Error>> {
    Ok(match std::env::var_os("XDG_CACHE_HOME") {
        Some(path) => PathBuf::from(path),
        None => dirs::home_dir()
            .ok_or("home directory unavailable")?
            .join(".cache"),
...
pub fn project_db_path(root: &Path) -> Result<PathBuf, Box<dyn std::error::Error>> {
    Ok(project_state_dir(root)?.join("lens.sqlite"))
}
