...
fn renders_native_paths_from_shared_cases() {
    for case in RENDER_CASES {
        ...
        let expected = match case.expected {
            RenderExpectation::RoundTrip(rendered) | RenderExpectation::RenderOnly(rendered) => {
                Ok(LegacyAppPathString::from_string(rendered))
            }
...
fn renders_an_absolute_path_using_the_host_convention() {
    ...
    assert_eq!(
        ...
        LegacyAppPathString::from_string(native_path)
    );
}
