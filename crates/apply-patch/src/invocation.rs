...
use crate::path_uri::PathConvention;
...
pub fn maybe_parse_apply_patch_verified(argv: &[String], cwd: &PathUri) -> MaybeApplyPatchVerified {
    ...
    match maybe_parse_apply_patch(argv, cwd) {
        MaybeApplyPatch::Body(args) => verify_apply_patch_args(args, cwd),
...
pub fn verify_apply_patch_args(args: ApplyPatchArgs, cwd: &PathUri) -> MaybeApplyPatchVerified {
    match try_verify_apply_patch_args(args, cwd) {
        Ok(action) => MaybeApplyPatchVerified::Body(action),
        Err(err) => MaybeApplyPatchVerified::CorrectnessError(err),
    }
}

fn try_verify_apply_patch_args(
    args: ApplyPatchArgs,
    cwd: &PathUri,
    ...
    for hunk in hunks {
        ...
        match hunk {
            ...
            Hunk::DeleteFile { .. } => {
                let content = crate::fs::read_file_text(&path).map_err(|source| {
...
PATCH"#,
    ...
    #[test]
    fn test_apply_patch_interception_uses_cwd_convention_for_windows_pwsh_path() {
        let script = heredoc_script("");
        assert_match_args_with_cwd(
            strs_to_strings(&[
...
+after"#
    ...
    #[test]
    fn test_unreadable_destinations_still_verify() {
        let session_dir = tempdir().unwrap();
        ...
        for argv in [add_argv, move_argv] {
            let result = maybe_parse_apply_patch_verified(&argv, &cwd);
    ...
    #[cfg(unix)]
    #[test]
    fn test_delete_symlink_still_verifies() {
