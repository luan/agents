...
use super::absolute_path::AbsolutePathBuf;
use super::absolute_path::normalize_windows_device_path;
use base64::Engine;
...
#[path = "path_uri/absolute_path_normalization.rs"]
mod absolute_path_normalization;
#[path = "path_uri/api_path_string.rs"]
mod api_path_string;
...
fn infer_opaque_path_convention(path_bytes: &[u8]) -> Option<PathConvention> {
    ...
    let (path_chunks, _) = path_bytes.as_chunks::<2>();
    let mut path_wide = path_chunks.iter().map(|bytes| u16::from_le_bytes(*bytes));
