...
use crate::path_uri::PathUriParseError;
use anyhow::Context;
use anyhow::Result;
pub use parser::Hunk;
...
pub fn apply_hunks(
    ...
    match apply_hunks_to_files(hunks, cwd, &mut delta) {
        Ok(affected_paths) => {
        ...
        match hunk {
            Hunk::AddFile { contents, .. } => {
                let overwritten_content =
                    read_optional_file_text_for_delta(&path_uri, &mut delta.exact);
            ...
            Hunk::DeleteFile { .. } => {
                note_existing_path_delta_support(&path_uri, &mut delta.exact);
                let deleted_content = crate::fs::read_file_text(&path_uri).ok();
                if deleted_content.is_none() {
                    delta.exact = false;
                }
                ensure_not_directory(&path_uri).with_context(|| {
                    format!(
                        "Failed to delete file {}",
                        path_uri.inferred_native_path_string()
                    )
                })?;
                if let Err(error) = crate::fs::remove(
                    &path_uri,
                    RemoveOptions {
                        recursive: false,
                        force: false,
                ...
                .with_context(|| {
                    ...
                    delta.exact &=
                        remove_failure_was_side_effect_free(&path_uri, deleted_content.as_deref());
                    return Err(error);
                }
            ...
            Hunk::UpdateFile {
                ...
                let AppliedPatch {
                    original_contents,
                    new_contents,
                } = derive_new_contents_from_chunks(&path_uri, chunks)?;
                if let Some(dest) = move_path {
                    let dest_uri = cwd.join(&dest.to_string_lossy())?;
                    let overwritten_move_content =
                        read_optional_file_text_for_delta(&dest_uri, &mut delta.exact);
                    try_write!(write_file_with_missing_parent_retry(
                        &dest_uri,
                    ...
                    try_write!(
                        crate::fs::write_file(&path_uri, new_contents.clone().into_bytes())
                            .with_context(|| format!(
...
fn ensure_not_directory(path: &PathUri) -> io::Result<()> {
    let metadata = crate::fs::get_metadata(path)?;
...
fn remove_failure_was_side_effect_free(path: &PathUri, expected_content: Option<&str>) -> bool {
    match expected_content {
        Some(expected_content) => {
            crate::fs::read_file_text(path).is_ok_and(|content| content == expected_content)
        }
        None => false,
    }
}

fn read_optional_file_text_for_delta(path: &PathUri, exact: &mut bool) -> Option<String> {
    note_existing_path_delta_support(path, exact);
    match crate::fs::read_file_text(path) {
...
fn note_existing_path_delta_support(path: &PathUri, exact: &mut bool) {
...
fn write_file_with_missing_parent_retry(path: &PathUri, contents: Vec<u8>) -> anyhow::Result<()> {
    match crate::fs::write_file(path, contents.clone()) {
        Ok(()) => Ok(()),
        Err(err) if err.kind() == io::ErrorKind::NotFound => {
            if let Some(parent) = path.parent() {
                crate::fs::create_directory(&parent, CreateDirectoryOptions { recursive: true })
                    .with_context(|| {
                        format!(
            ...
            crate::fs::write_file(path, contents).with_context(|| {
                format!(
                    "Failed to write file {}",
                    path.inferred_native_path_string()
                )
...
fn derive_new_contents_from_chunks(
    ...
    let original_contents = crate::fs::read_file_text(path).map_err(|err| {
        ApplyPatchError::IoError(IoError {
            context: format!(
...
+cd"#,
    ...
    #[test]
    fn test_apply_patch_hunks_accept_relative_and_absolute_paths() {
        let dir = tempdir().unwrap();
...
+absolute new"#,
        ...
        apply_patch(&patch, &cwd, &mut stdout, &mut stderr).unwrap();
...
+line2"#,
    ...
    #[cfg(unix)]
    #[test]
    fn test_failed_move_returns_committed_destination_delta() {
...
+QUX"#,
        ...
        let diff = unified_diff_from_chunks(&path_uri, update_file_chunks).unwrap();
...
"#,
        ...
        let diff = unified_diff_from_chunks(&resolved_path, chunks).unwrap();
...
"#,
        ...
        let path_uri = PathUri::from_host_native_path(&path).expect("absolute test path");
        let diff = unified_diff_from_chunks(&path_uri, chunks).unwrap();
...
"#
    ...
    #[test]
    fn test_unreadable_destinations_return_inexact_delta() {
        let dir = tempdir().unwrap();
        ...
        for patch in [
            ...
            let mut stdout = Vec::new();
            let mut stderr = Vec::new();
            let delta = apply_patch(&patch, &cwd, &mut stdout, &mut stderr).unwrap();
    ...
    #[cfg(unix)]
    #[test]
    fn test_delete_symlink_returns_inexact_delta() {
