...
// The probe panics on purpose. Unwinding out of the panic aborts this v8-linked test binary
// with "failed to initiate panic, error 5". Predates the workspace move; see UPSTREAM.md.
#[ignore = "panic probe aborts the v8-linked test binary"]
#[tokio::test]
...
#[tokio::test]
// `await_holding_invalid_type` only fires with an `await-holding-invalid-types` clippy.toml, which
// this workspace does not carry, so this cannot be `expect`.
#[allow(
