...
3. If Git-Spice reports restack conflicts, use the `gs:restack` skill to resolve them.
4. Report the remote update, deleted merged branches, and branches synchronized.

`gs repo sync` pulls latest changes from the remote. With `--restack`, it restacks the current stack after syncing. It does not replace the `gs:submit` skill for pushing or Change Request updates.
