...
3. If Graphite reports restack conflicts, use the `gt:restack` skill to resolve them.
4. Report the trunk update and branches synchronized.

`gt sync` pulls trunk and restacks. It does not push the stack; use the `gt:submit` skill for pushing or PR updates.
