...
export function registerExplorationRenderContext(toolCallId: string | undefined, invalidate: (() => void) | undefined): void {
...
export function getExplorationRenderInfo(toolCallId: string | undefined, fallbackStatus: ExplorationStatus): ExplorationRenderInfo {
	...
	if (!group) {
		return { hidden: false, status: entry.status, actionGroups: [[entry.action]] };
	}
...
export function renderExplorationCall(
	action: ExplorationAction,
	theme: ExplorationRenderTheme,
	context: { toolCallId?: string; invalidate?: () => void; isPartial?: boolean; isError?: boolean } | undefined,
): string {
	...
	return renderExplorationText(renderInfo.actionGroups ?? [[action]], renderInfo.status, theme, context?.isError === true);
}
...
function renderStatusMarker(marker: string, status: ExplorationStatus, theme: Pick<ExplorationRenderTheme, "fg">, failed: boolean): string {
