...
export function registerExplorationTool(
  toolName: string,
  toAction: ArgsToAction,
): void {
...
export function registerExplorationEventHandlers(pi: PiWithEvents): void {
  ...
  pi.on("tool_execution_end", (event) => {
    if (actionByToolName.has(event.toolName))
      recordExplorationEnd(event.toolCallId);
  });
}
...
export function getExplorationRenderInfo(
  ...
  const entries = group.entryIds
    ...
    .filter((groupEntry): groupEntry is ExplorationEntry =>
      Boolean(groupEntry),
    );
  return {
    hidden: false,
    status: entries.some((groupEntry) => groupEntry.status === "running")
      ? "running"
      : "done",
...
export function renderExplorationText(
  ...
  for (const [index, action] of coalesceReadActions(
    actionGroups.flat(),
  ).entries()) {
...
export function renderExplorationCall(
  ...
  const renderInfo = getExplorationRenderInfo(
    context?.toolCallId,
    fallbackStatus,
  );
...
function recordExplorationStart(
  toolCallId: string,
  action: ExplorationAction,
): void {
  ...
  let group = activeExplorationGroupId
    ? groupsById.get(activeExplorationGroupId)
    : undefined;
  if (!group) {
    group = {
      id: nextGroupId++,
...
function getGroupForEntry(
  entry: ExplorationEntry | undefined,
): ExplorationGroup | undefined {
...
function coalesceReadActions(
  ...
  for (let index = 0; index < actions.length; index += 1) {
    ...
    coalesced.push({
      ...
      body: reads
        .map((read) =>
          duplicateBodies.has(read.body) ? (read.path ?? read.body) : read.body,
        )
        .join(", "),
