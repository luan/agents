...
test("exploration grouping coalesces read and search tool calls", () => {
	...
	registerExplorationTool("read", (args) => readAction(typeof args === "object" && args && "path" in args ? String(args.path) : ""));
	...
	emit("session_start", {});
	emit("tool_execution_start", { toolName: "read", toolCallId: "read-1", args: { path: "/tmp/a.ts" } });
	emit("tool_execution_start", { toolName: "grep", toolCallId: "grep-1", args: { pattern: "needle" } });
	...
	expect(
		renderExplorationCall(
			{ kind: "search", title: "Search", body: "needle" },
			theme,
			{ toolCallId: "grep-1", isPartial: false },
		),
	).toBe("• **Explored**\n  └ Read a.ts\n    Search needle");
});
