...
export type LensSeverity = "pending" | "clean" | "warning" | "degraded" | "error" | "unknown";
...
export function lensSeverity(value: unknown): LensSeverity {
	...
	if (health === "pending" || health === "collecting" || health === "running") return "pending";
...
export function renderLensWidgetLines(value: unknown, expanded = false, options: LensRenderOptions = {}): string[] {
	...
	const actionContext = asRecord(
		data?.context ?? data?.data?.action_context ?? data?.data?.health?.action_context ?? data?.data,
	);
	const progress = progressLines(data).map((line) => colorDetailLine(line, style));
...
function actionLines(data: LensRecord | undefined, actionContext: LensRecord | undefined): string[] {
	...
	if ((cleanup?.failed ?? 0) > 0 || (cleanup?.timed_out ?? 0) > 0)
		out.push(`  cleanup: inspect ${cleanup.failed ?? 0} failed/${cleanup.timed_out ?? 0} timed-out run(s)`);
	const diagnostics = asRecord(summary?.diagnostics ?? data?.diagnostics);
	if ((diagnostics?.errors ?? 0) > 0 || (diagnostics?.warnings ?? 0) > 0)
		out.push(`  diagnostics: resolve ${diagnostics.errors ?? 0} error(s), ${diagnostics.warnings ?? 0} warning(s)`);
	const checks = asRecord(summary?.checks ?? data?.checks);
	const failingChecks = Array.isArray(checks?.latest)
		? checks.latest.filter((check: LensRecord) => (check?.exit_code ?? 0) !== 0 || (check?.diagnostic_count ?? 0) > 0)
				.length
		: 0;
...
function hasHealthSummary(data: LensRecord | undefined): boolean {
	return !!asRecord(data?.data?.health?.summary ?? data?.data?.summary ?? data?.health?.details);
}
...
function checksSummary(data: LensRecord | undefined): string | undefined {
	...
	if (runs.length > 0) {
		const failed = runs.filter((run) => (run?.exit_code ?? 0) !== 0 || (run?.diagnostic_count ?? 0) > 0).length;
		return failed > 0 ? `checks ${runs.length} ran/${failed} issue` : `checks ${runs.length} ran`;
	}
	...
	const errors = latest.filter(
		(check: LensRecord) =>
			typeof check?.exit_code === "number" && check.exit_code !== 0 && (check?.diagnostic_count ?? 0) > 0,
	).length;
	const warnings = latest.filter(
		(check: LensRecord) => check?.exit_code === null && (check?.diagnostic_count ?? 0) > 0,
	).length;
...
function progressSummary(data: LensRecord | undefined): string | undefined {
	...
	const parts = [queue.length > 0 ? `queue: ${queue.join(", ")}` : "end: cleanup"];
	const suggestions = arrayValue(plan.suggestions);
	if (suggestions.length > 0) parts.push(`suggested: ${suggestions.join(", ")}`);
	return parts.join(" · ");
}

function progressLines(data: LensRecord | undefined): string[] {
	const out: string[] = [];
	const healthSummary = asRecord(data?.data?.health?.summary ?? data?.data?.summary ?? data?.health?.details);
	const changed = asRecord(healthSummary?.changed_files);
	const changedCount = numberValue(changed?.count);
	const plan = validationPlan(data);
	if (plan?.turn_active) {
		if (changedCount !== undefined) out.push(`  collecting: ${changedCount} touched file(s)`);
		...
		out.push(planned.length > 0 ? `  queue: ${planned.join(", ")}` : "  end: cleanup");
		if (suggestions.length > 0) out.push(`  suggested: ${suggestions.join(", ")}`);
	}
	const runs = turnCheckRuns(data);
	if (runs.length > 0) out.push(`  checks ran: ${runs.map(formatCheckRun).join(", ")}`);
	return out;
}

function lspSummary(data: LensRecord | undefined): string | undefined {
	const lsp = asRecord(data?.data?.health?.summary?.lsp ?? data?.data?.summary?.lsp);
	const connected = Array.isArray(lsp?.connected) ? lsp.connected : [];
	const names = connected.map((item) => stringValue(asRecord(item)?.name)).filter((name): name is string => !!name);
...
function validationQueue(plan: LensRecord): string[] {
	return [...arrayValue(plan.automatic_checks), ...arrayValue(plan.automatic_scanners)];
}

function turnCheckRuns(data: LensRecord | undefined): LensRecord[] {
	const runs = data?.data?.turn?.checks?.runs ?? data?.data?.checks?.runs;
	return Array.isArray(runs) ? runs.filter((run): run is LensRecord => !!asRecord(run)) : [];
}
