...
const USER_MESSAGE_ORIGINAL_RENDER = Symbol.for(
	"agents.polishedTui.userMessageOriginalRender",
);
const CUSTOM_EDITOR_ORIGINAL_RENDER = Symbol.for(
	"agents.polishedTui.customEditorOriginalRender",
);
  ...
  const prototype = UserMessageComponent.prototype as unknown as {
    ...
    if (baseLines.length === 0) return baseLines;
		...
		const styledLines = contentLines.map(
			(line) =>
				`${rail}${fillBackgroundLine(uiTheme, line, innerWidth)}`,
		);
...
function renderPolishedEditor(
	...
	const isShowingAutocomplete =
		typeof editor.isShowingAutocomplete === "function"
			? Boolean(editor.isShowingAutocomplete())
			: false;
	...
	return [
		...lines.map(
			(line) => `${rail}${fillBackgroundLine(uiTheme, line, innerWidth)}`,
		),
		...autocompleteLines,
	];
}
...
export function installEditorComposition(uiTheme: Theme): void {
	...
	const originalRender =
		(prototype[CUSTOM_EDITOR_ORIGINAL_RENDER] as
			| ((this: CustomEditor, width: number) => string[])
			| undefined) ?? prototype.render;
	...
	prototype.render = function (this: CustomEditor, width: number): string[] {
		return renderPolishedEditor(this as unknown as TransformableEditor, width, (w) =>
