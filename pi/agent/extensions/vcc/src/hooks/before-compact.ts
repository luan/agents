...
import { loadSettings, type VccSettings } from "../core/settings";
import { compile } from "../core/summarize";
import type { VccCompactionDetails } from "../details";

export const VCC_COMPACT_INSTRUCTION = "__vcc__";
...
const dbg = (settings: VccSettings, data: Record<string, unknown>) => {
	if (!settings.debug) return;
	try {
		writeFileSync("/tmp/vcc-debug.json", JSON.stringify(data, null, 2));
	} catch {}
};
...
export const registerBeforeCompactHook = (pi: ExtensionAPI) => {
	pi.on("session_before_compact", (event, ctx) => {
		...
		// Always handle explicit /vcc marker.
		// Otherwise, only handle when user opted in via settings.
		const isVcc = customInstructions === VCC_COMPACT_INSTRUCTION;
		if (!isVcc && !settings.overrideDefaultCompaction) return;
				...
				return (
					...
					c.reduce((s: number, p: any) => {
						...
						if (p.type === "toolResult")
							return (
								s + (typeof p.content === "string" ? p.content.length : JSON.stringify(p.content ?? "").length)
							);
						return s;
					}, 0)
				);
		...
		const details: VccCompactionDetails = {
			compactor: "vcc",
			version: 1,
			sections: [...summary.matchAll(/^\[(.+?)\]/gm)].map((m) => m[1]),
	...
	// /vcc path uses its own onComplete callback in the command handler.
	pi.on("session_compact", (event, ctx) => {
		if (!event.fromExtension) return;
		if (lastCompactWasVcc) return; // /vcc handles its own toast via onComplete
