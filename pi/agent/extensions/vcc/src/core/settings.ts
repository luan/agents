...
export const SETTINGS_PATH_DEFAULT = join(homedir(), ".pi", "agent", "vcc-config.json");
const settingsPath = (): string => process.env.VCC_CONFIG_PATH ?? SETTINGS_PATH_DEFAULT;
...
export interface VccSettings {
	/**
	 * When true, vcc handles ALL compactions:
	 *   - /compact (no args)
	 *   - /compact <text>
	 *   - auto threshold / overflow
	 *   - /vcc (always handled regardless)
	 *
	 * When false (default), vcc only handles /vcc; everything else
	 * falls back to pi core's default LLM-based compaction.
	 */
	overrideDefaultCompaction: boolean;
	/** Write debug snapshot to /tmp/vcc-debug.json on each compaction. */
	debug: boolean;
}
...
export function loadSettings(): VccSettings {
	const parsed = readJson(settingsPath());
	if (!parsed || typeof parsed !== "object") return { ...DEFAULT_SETTINGS };
	return { ...DEFAULT_SETTINGS, ...(parsed as Partial<VccSettings>) };
}

/**
 * Ensure ~/.pi/agent/vcc-config.json exists with default keys.
