...
function readRenderAction(
  cwd: string,
  args: { path?: string; offset?: unknown; limit?: unknown },
) {
  const path = args.path ? resolve(cwd, args.path) : "file";
...
export default function piPrettyExtension(
  ...
  registerExplorationTool("read", (args) => {
    ...
    return readRenderAction(cwd, {
      path,
      offset:
        args && typeof args === "object" && "offset" in args
          ? args.offset
          : undefined,
      limit:
  ...
  pi.registerTool({
    ...
    renderCall(args: ReadParams, theme: ThemeLike, ctx: RenderContextLike) {
      ...
      text.setText(
        renderExplorationCall(readRenderAction(cwd, args), theme, ctx),
      );
      return text;
    },
