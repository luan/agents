...
import type { FffGrepCursor, FffGrepMatch } from "./fff-types";
...
export class CursorStore {
  private cursors = new Map<string, FffGrepCursor>();
  private counter = 0;
  ...
  store(cursor: FffGrepCursor): string {
  ...
  get(id: string): FffGrepCursor | undefined {
 ...
 */
export function fffFormatGrepText(
  items: FffGrepMatch[],
  limit: number,
): string {
  ...
  for (const match of capped) {
    ...
    if (match.contextBefore?.length) {
      ...
      for (let i = 0; i < match.contextBefore.length; i++) {
        lines.push(
          `${match.relativePath}-${startLine + i}-${match.contextBefore[i]}`,
        );
      }
    }
    const content =
      match.lineContent.length > 500
        ? `${match.lineContent.slice(0, 500)}...`
        : match.lineContent;
    ...
    if (match.contextAfter?.length) {
      ...
      for (let i = 0; i < match.contextAfter.length; i++) {
        lines.push(
          `${match.relativePath}-${startLine + i}-${match.contextAfter[i]}`,
        );
      }
    }
  }
