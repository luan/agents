...
function parseArgs(input: string): ParseResult | undefined {
	...
	if (trailing) {
		...
		return { intervalMs: parseInterval(`${trailing[2]}${unit}`)!, prompt: trailing[1].trim() };
	}
		...
		handler: async (args, ctx) => {
			...
			if (trimmed === "list") {
				...
				const lines = [...loops.values()].map(
					(loop) => `• ${loop.id} every ${formatInterval(loop.intervalMs)} fires ${loop.fireCount}/${loop.maxFires}: ${loop.prompt}`,
				);
