...
function parseArgs(input: string): ParseResult | undefined {
  ...
  if (leading !== undefined)
    return { intervalMs: leading, prompt: rest.join(" ").trim() };
    ...
    handler: async (args, ctx) => {
      ...
      if (trimmed === "stop") {
        ...
        return ctx.ui.notify(
          count === 0 ? "No active loops." : `Cancelled ${count} loop(s).`,
          "info",
        );
      }
      ...
      if (trimmed.startsWith("stop ")) {
        ...
        if (!entry)
          return ctx.ui.notify(`No loop found with id ${id}.`, "warning");
        cancel(entry);
      ...
      const sendPrompt = () => {
        ...
        if (entry.fireCount > entry.maxFires) {
          cancel(entry);
          ctx.ui.notify(
            `Loop ${id} stopped after ${entry.maxFires} fires.`,
            "info",
          );
          return;
        }
      ...
      const entry: LoopEntry = {
        ...
        expiryTimer: setTimeout(() => {
          ...
          cancel(current);
          ctx.ui.notify(
            `Loop ${id} expired after ${formatInterval(MAX_AGE_MS)}.`,
            "info",
          );
        }, MAX_AGE_MS),
      };
      ...
      ctx.ui.notify(
        `Scheduled ${id}: every ${formatInterval(intervalMs)} — stop with /loop stop ${id}`,
        "info",
      );
      sendPrompt();
    },
