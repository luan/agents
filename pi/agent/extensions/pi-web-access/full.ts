...
function handleSessionChange(ctx: ExtensionContext): void {
	abortPendingFetches();
	closeCurator();
	void import("./github-extract.js").then(({ clearCloneCache }) => clearCloneCache()).catch(() => {});
	sessionActive = true;
...
export default function (pi: ExtensionAPI) {
	...
	async function openCuratorBrowser(pc: PendingCurate, searchesComplete = true): Promise<void> {
		...
		try {
			pc.phase = "curating";
			const { startCuratorServer } = await import("./curator-server.js");
	...
	pi.registerTool({
		...
		async execute(toolCallId, params, signal) {
			const { executeCodeSearch } = await import("./code-search.js");
