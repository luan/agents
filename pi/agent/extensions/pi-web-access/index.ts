...
function loadShortcutConfig(): { curate: string; activity: string } {
	...
	try {
		const config = JSON.parse(readFileSync(WEB_SEARCH_CONFIG_PATH, "utf-8")) as {
			shortcuts?: { curate?: unknown; activity?: unknown };
		};
		return {
			curate: typeof config.shortcuts?.curate === "string" ? config.shortcuts.curate : DEFAULT_SHORTCUTS.curate,
			activity: typeof config.shortcuts?.activity === "string" ? config.shortcuts.activity : DEFAULT_SHORTCUTS.activity,
		};
	} catch {
...
function captureApi(pi: ExtensionAPI, runtime: CapturedRuntime): ExtensionAPI {
	return {
		...
		sendUserMessage: (content: any, options?: any) => pi.sendUserMessage(content, options),
		appendEntry: (customType: string, data?: any) => pi.appendEntry(customType, data),
		...
		registerProvider: (name: string, config: any) => pi.registerProvider(name, config),
		unregisterProvider: (name: string) => pi.unregisterProvider(name),
		events: pi.events,
	} as ExtensionAPI;
}

async function ensureRuntime(pi: ExtensionAPI, ctx?: ExtensionContext): Promise<CapturedRuntime> {
	if (!runtimePromise) {
...
async function runHandlers(runtime: CapturedRuntime, event: string, payload: any, ctx: ExtensionContext) {
...
function isComponent(value: unknown): value is { render(width: number): string[] } {
...
function registerLazyTool(pi: ExtensionAPI, name: string, definition: any) {
	pi.registerTool({
		...
		async execute(toolCallId: string, params: any, signal: AbortSignal | undefined, onUpdate: any, ctx: ExtensionContext) {
			...
			if (!tool) throw new Error(`pi-web-access internal error: ${name} did not register.`);
...
export default function piWebAccess(pi: ExtensionAPI) {
	registerLazyTool(pi, "web_search", {
		name: "web_search",
		label: "Web Search",
		description: "Search the web using Perplexity AI, Exa, or Gemini. Returns an AI-synthesized answer with source citations.",
		...
		parameters: Type.Object({
			...
			workflow: Type.Optional(StringEnum(["none", "summary-review"], { description: "Search workflow mode: none = no curator, summary-review = open curator" })),
		}),
	});
	...
	registerLazyTool(pi, "code_search", {
		...
		parameters: Type.Object({
			...
			maxTokens: Type.Optional(Type.Number({ description: "Maximum tokens of code/documentation context to return (default: 5000)" })),
		}),
	});
	...
	registerLazyTool(pi, "fetch_content", {
		name: "fetch_content",
		label: "Fetch Content",
		description: "Fetch URL(s) and extract readable content as markdown. Supports GitHub repos, PDFs, YouTube videos, and local video files.",
		promptSnippet: "Use to extract readable content from URL(s), YouTube, GitHub repos, or local videos.",
		parameters: Type.Object({
			...
			prompt: Type.Optional(Type.String({ description: "Question or instruction for video analysis" })),
			timestamp: Type.Optional(Type.String({ description: "Extract video frame(s) at a timestamp or time range" })),
			frames: Type.Optional(Type.Number({ description: "Number of frames to extract" })),
	...
	for (const name of ["websearch", "curator", "google-account", "search"]) {
		pi.registerCommand(name, {
			description: name === "websearch" ? "Open web search curator" : `pi-web-access ${name}`,
			handler: async (args, ctx) => {
				const runtime = await ensureRuntime(pi, ctx);
				const command = runtime.commands.get(name);
				if (!command) throw new Error(`pi-web-access internal error: /${name} did not register.`);
	...
	pi.on("session_tree", async (event, ctx) => {
		if (loadedRuntime) await runHandlers(loadedRuntime, "session_tree", event, ctx);
	});

	pi.on("session_shutdown", async (event, ctx) => {
		if (loadedRuntime) await runHandlers(loadedRuntime, "session_shutdown", event, ctx);
	});
}
