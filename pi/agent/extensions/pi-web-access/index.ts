...
function loadShortcutConfig(): { curate: string; activity: string } {
	...
	try {
		...
		return {
			...
			activity:
				typeof config.shortcuts?.activity === "string" ? config.shortcuts.activity : DEFAULT_SHORTCUTS.activity,
		};
	} catch {
		...
		parameters: Type.Object({
			...
			queries: Type.Optional(
				Type.Array(Type.String(), {
					description: "Multiple queries searched in sequence, each returning its own synthesized answer.",
				}),
			),
			...
			workflow: Type.Optional(
				StringEnum(["none", "summary-review"], {
					description: "Search workflow mode: none = no curator, summary-review = open curator",
				}),
			),
		}),
	});
