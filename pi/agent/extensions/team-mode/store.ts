...
import type { TeamEvent, TeamRecord, TeamSummary, TeamTask, WorkerRecord, WorkerRole, WorkerStatus } from "./types.js";
...
export class TeamStore {
	...
	async createTeam(objective: string, cwd: string): Promise<TeamRecord> {
		...
		await this.appendEvent(team.id, { type: "team_started", message: `Team started: ${objective}` });
		return team;
	}

	async saveTeam(team: TeamRecord): Promise<void> {
		await writeJsonAtomic(this.teamFile(team.id), { ...team, updatedAt: nowIso() });
	}
	...
	async listTeams(): Promise<TeamRecord[]> {
		...
		return teams.filter((team): team is TeamRecord => team !== null).sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));
	}
	...
	async saveWorker(worker: WorkerRecord): Promise<void> {
		await writeJsonAtomic(this.workerFile(worker.teamId, worker.id), { ...worker, updatedAt: nowIso() });
	}
	...
	async createTask(teamId: string, input: CreateTaskInput): Promise<TeamTask> {
		...
		await this.appendEvent(teamId, { type: "task_created", taskId: task.id, message: `Task created: ${task.subject}` });
		return task;
	}
	...
	async updateTask(teamId: string, taskId: string, patch: TaskPatch, expectedVersion: number): Promise<TeamTask> {
		return withLock(this.teamLock(teamId, `task-${taskId}`), async () => {
			...
			await this.appendEvent(teamId, { type: "task_updated", taskId, message: `Task updated: ${updated.subject}` });
			return updated;
		});
	}
	...
	async appendEvent(teamId: string, input: Omit<TeamEvent, "id" | "teamId" | "createdAt">): Promise<TeamEvent> {
		...
		const event: TeamEvent = { id: `event-${Date.now()}-${randomUUID().slice(0, 8)}`, teamId, createdAt: nowIso(), ...input };
	...
	async summary(teamId: string): Promise<TeamSummary | null> {
		...
		if (!team) return null;
		return { team, workers: await this.listWorkers(teamId), tasks: await this.listTasks(teamId), events: await this.listEvents(teamId) };
	}
