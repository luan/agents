...
export function getTeamModeRoot(): string {
  return (
    process.env.PI_TEAM_MODE_ROOT ||
    join(homedir(), ".pi", "agent", "team-mode")
  );
}
...
async function listJson<T>(dir: string): Promise<T[]> {
  ...
  const items = await Promise.all(
    files.map((file) => readJson<T>(join(dir, file))),
  );
...
export type TaskPatch = Partial<
  Pick<
    TeamTask,
    "subject" | "description" | "status" | "owner" | "files" | "result"
  >
> & {
  dependencies?: string[];
  blocks?: string[];
...
export class TeamStore {
  ...
  async latestActiveTeam(): Promise<TeamRecord | null> {
    return (
      (await this.listTeams()).find(
        (team) => team.status === "running" || team.status === "paused",
      ) ?? null
    );
  }
      ...
      if (
        existing.some(
          (worker) =>
            ...
            !TERMINAL_WORKER_STATUSES.has(worker.status),
        )
      ) {
  ...
  async loadWorker(
    ...
    const direct = await readJson<WorkerRecord>(
      this.workerFile(teamId, workerIdOrName),
    );
    if (direct) return direct;
    return (
      (await this.listWorkers(teamId)).find(
        (worker) => worker.name === workerIdOrName,
      ) ?? null
    );
  }
  ...
  async appendEvent(
    teamId: string,
    input: Omit<TeamEvent, "id" | "teamId" | "createdAt">,
  ): Promise<TeamEvent> {
  ...
  async listEvents(teamId: string, limit = 100): Promise<TeamEvent[]> {
    try {
      const lines = (await readFile(this.eventsFile(teamId), "utf8"))
        .split("\n")
        .filter(Boolean);
  ...
  async clearTeam(teamId: string): Promise<void> {
    if (await exists(this.teamDir(teamId)))
  ...
  async reconcileRunningWorkers(teamId: string): Promise<void> {
    ...
    for (const worker of workers) {
      ...
      worker.lastSummary =
        "Marked stopped during startup recovery; live process is not attached.";
      await this.saveWorker(worker);
      await this.appendEvent(teamId, {
        type: "worker_failed",
        actor: worker.name,
