...
export class PiSubagentRuntime implements TeamRuntime {
  ...
  resumeWorker(
    ...
    return this.start(
      worker,
      task,
      message ?? "Continue from the last checkpoint and report progress.",
    );
  }
  ...
  private async start(
    ...
    if (this.live.has(worker.id))
      throw new Error(`worker already running: ${worker.name}`);
    ...
    const sessionFile =
      launchWorker.sessionFile ?? this.store.workerSessionFile(launchWorker.id);
    ...
    const proc = spawn("pi", args, {
      cwd: launchWorker.cwd,
      stdio: ["ignore", "pipe", "pipe"],
      env: { ...process.env, PI_TEAM_MODE_WORKER: "1" },
    });
    const updated: WorkerRecord = {
      ...launchWorker,
      status: "running",
      sessionFile,
      asyncRunId: proc.pid ? String(proc.pid) : undefined,
    };
    await this.store.saveWorker(updated);
  ...
  private async finishRun(
    ...
    const sessionText = worker.sessionFile
      ? await readFile(worker.sessionFile, "utf8").catch(() => "")
      : "";
    const summary =
      extractFinalText(sessionText) ||
      extractFinalText(stdout) ||
      (failed ? stderr.trim() : "") ||
      (failed ? `pi exited ${code ?? signal}` : "Worker completed.");
    ...
    this.onEvent({
      worker: next,
      status: next.status as "completed" | "failed",
      summary,
    });
  }
}
...
async function createWorkerWorktree(
  ...
  const repoRoot = (
    await runGit(worker.cwd, ["rev-parse", "--show-toplevel"])
  ).trim();
  ...
  const branch = `team-mode/${worker.name}-${suffix}`.replace(
    /[^A-Za-z0-9/_-]/g,
    "-",
  );
