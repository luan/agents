...
export class TeamDashboard {
  ...
  render(width: number): string[] {
    ...
    const push = (line: string) =>
      lines.push(truncateToWidth(line, w, "…", true));
    ...
    push(
      this.row(
        `${this.label("Status")} ${team.status}    Tasks ${doneTasks}/${tasks.length}    Workers ${workers.length}`,
        w,
      ),
    );
    ...
    push(
      this.borderBottom(
        " ↑↓ move  tab panes  enter inspect  m message  p pause  s stop  r refresh  esc ",
        w,
      ),
    );
  ...
  private inspectSelected(): void {
    ...
    if (this.pane === "workers" && row)
      this.onAction({ type: "message", worker: row as WorkerRecord });
  }
  ...
  private stopSelected(): void {
    ...
    this.onAction({
      type: "stop",
      worker:
        this.pane === "workers" ? (row as WorkerRecord | undefined) : undefined,
    });
  }
  ...
  private openSelectedPath(): void {
    ...
    if (this.pane === "tasks") {
      ...
      if (task?.files[0])
        this.onAction({ type: "openPath", path: task.files[0] });
    }
  }
  ...
  private taskLines(width: number): string[] {
    ...
    return tasks.map((task, index) => {
      ...
      const prefix = selected ? ">" : " ";
      return this.row(
        `${prefix} ${TASK_STATUS_ICON[task.status]} ${task.id.slice(0, 14).padEnd(14)} ${task.subject.padEnd(36)} ${task.owner ?? task.status}`,
        width,
        selected,
      );
    });
  ...
  private eventLines(events: TeamEvent[], width: number): string[] {
    ...
    return list.map((event, index) => {
      ...
      return this.row(
        `${selected ? ">" : " "} ${time} ${event.actor ? `${event.actor}: ` : ""}${event.message}`,
        width,
        selected,
      );
    });
  ...
  private detailLine(width: number): string {
    ...
    if (this.pane === "workers" && row)
      text =
        (row as WorkerRecord).lastResult ??
        (row as WorkerRecord).sessionFile ??
        "";
    if (this.pane === "tasks" && row) text = (row as TeamTask).description;
    if (this.pane === "events" && row)
      text = JSON.stringify((row as TeamEvent).details ?? {});
...
export function renderCompactWidget(
  ...
  const lines = [
    ...
    ...summary.workers
      ...
      .map(
        (worker) =>
          `  ${WORKER_STATUS_ICON[worker.status]} ${worker.name}: ${worker.lastSummary ?? worker.status}`,
      ),
  ];
  return lines.map((line, index) =>
    truncateToWidth(
      index === 0 ? theme.fg("accent", line) : line,
      width,
      "…",
      true,
    ),
