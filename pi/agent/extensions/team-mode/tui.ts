...
import type { TeamEvent, TeamSummary, TeamTask, WorkerRecord } from "./types.js";
...
export class TeamDashboard {
	...
	private stopSelected(): void {
		...
		this.onAction({
			type: "stop",
			worker: this.pane === "workers" ? (row as WorkerRecord | undefined) : undefined,
		});
	}
	...
	private detailLine(width: number): string {
		...
		if (this.pane === "workers" && row)
			text = (row as WorkerRecord).lastResult ?? (row as WorkerRecord).sessionFile ?? "";
...
export function renderCompactWidget(summary: TeamSummary, theme: ThemeLike, width: number): string[] {
	...
	const lines = [
		...
		...summary.workers
			.slice(0, 3)
			.map(
				(worker) => `  ${WORKER_STATUS_ICON[worker.status]} ${worker.name}: ${worker.lastSummary ?? worker.status}`,
			),
	];
