...
import type { TeamEvent, TeamRecord, TeamSummary, TeamTask, WorkerMode, WorkerRecord, WorkerRole } from "./types.js";
...
const TeamTaskUpdateParams = Type.Object({
	...
	status: Type.Optional(
		StringEnum(["pending", "ready", "in_progress", "blocked", "review", "completed", "failed", "cancelled"] as const),
	),
...
export default function teamModeExtension(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		async execute(_id, raw, _signal, _update, ctx) {
			...
			const task = worker.currentTaskId
				? (await state.store.listTasks(team.id)).find((item) => item.id === worker.currentTaskId)
				: undefined;
		...
		async execute(_id, raw, _signal, _update, ctx) {
			...
			try {
				...
				if (error instanceof VersionConflictError) {
					return textResult(`Version conflict: task is v${error.actual}, retry with a fresh team_status.`, {
						conflict: true,
						actual: error.actual,
					});
				}
	...
	pi.registerCommand("team", {
		...
		handler: async (args, ctx) => {
			...
			if (sub === "status")
				return ctx.ui.notify(formatStatus(await state.store.summary((await activeTeam(ctx)).id)), "info");
			if (sub === "tasks")
				return ctx.ui.notify(
					formatTasks((await state.store.summary((await activeTeam(ctx)).id))?.tasks ?? []),
					"info",
				);
...
function sendTeamNotification(pi: ExtensionAPI, event: TeamEvent, worker?: WorkerRecord): void {
	pi.sendMessage(
		...
		{
			triggerTurn:
				event.type === "worker_completed" || event.type === "worker_failed" || event.type === "attention_needed",
		},
	);
}
...
async function controlTeam(
	...
	await state.store.appendEvent(team.id, {
		type: action === "pause" ? "team_paused" : action === "resume" ? "team_resumed" : "team_stopped",
		message: `Team ${action}`,
	});
}
...
async function clearCurrentTeam(ctx: ExtensionContext, state: RuntimeState): Promise<void> {
	...
	if (activeWorkers.length > 0)
		return ctx.ui.notify("Cannot clear a team with active workers. Stop them first.", "warning");
