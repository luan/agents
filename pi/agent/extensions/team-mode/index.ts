...
import {
  applyCoordinatorToolPolicy,
  defaultModeForRole,
  isBlockedCoordinatorTool,
  isGitDirty,
} from "./policy.js";
import { COORDINATOR_PROMPT, formatTeamNotification } from "./prompts.js";
import { PiSubagentRuntime } from "./runtime.js";
import { type TaskPatch, TeamStore, VersionConflictError } from "./store.js";
import {
  renderCompactWidget,
  TeamDashboard,
...
const WorkerRoleSchema = StringEnum([
  "researcher",
  "planner",
  "implementer",
  "reviewer",
...
const WorkerModeSchema = StringEnum([
  "advisory",
  "single-writer",
  "worktree",
] as const);
const StatusScopeSchema = StringEnum([
  "summary",
  "workers",
  "tasks",
  "events",
...
const TeamStartParams = Type.Object({
  ...
  template: Type.Optional(
    StringEnum(["research", "plan", "implement", "review", "custom"] as const),
  ),
});
...
const TeamStatusParams = Type.Object({
  scope: Type.Optional(StatusScopeSchema),
});
...
export default function teamModeExtension(pi: ExtensionAPI) {
  ...
  async function activeTeam(ctx: ExtensionContext): Promise<TeamRecord> {
    const team = state.activeTeamId
      ? await state.store.loadTeam(state.activeTeamId)
      : await state.store.latestActiveTeam();
    if (!team)
      throw new Error("No active team. Use /team start <objective> first.");
  ...
  async function spawnWorker(
    ...
    if (mode === "worktree" && (await isGitDirty(pi, team.cwd))) {
      throw new Error(
        "Cannot spawn worktree worker from a dirty checkout. Commit/stash changes or use single-writer mode.",
      );
    }
    const task = await createOrGetTask(
      team.id,
      params.task,
      params.task,
      params.files,
    ...
    await state.store.appendEvent(team.id, {
      type: "worker_spawned",
      actor: launched.name,
      taskId: task.id,
      message: `${launched.name} started ${task.subject}`,
    });
    await updateWidget(ctx, state, team.id);
    return launched;
  }
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use team_start before spawning named workers for long-running team orchestration.",
    ],
  ...
  pi.registerTool({
    ...
    description:
      "Spawn a named background worker. Returns immediately; completion arrives as a team notification.",
    promptSnippet: "Spawn a named background team worker",
    promptGuidelines: [
      "Workers must receive self-contained tasks. Do not predict worker results.",
    ],
    ...
    async execute(_id, raw, _signal, _update, ctx) {
      ...
      return textResult(`Worker spawned: ${worker.name} (${worker.id})`, {
        worker,
      });
    },
  } as any);
  ...
  pi.registerTool({
    ...
    description:
      "Resume/message a named worker with a self-contained follow-up.",
    ...
    async execute(_id, raw, _signal, _update, ctx) {
      ...
      const task = worker.currentTaskId
        ? (await state.store.listTasks(team.id)).find(
            (item) => item.id === worker.currentTaskId,
          )
        : undefined;
      const updated = await state.runtime.resumeWorker(
        worker,
        task,
        params.message,
      );
      await updateWidget(ctx, state, team.id);
      return textResult(`Message sent to ${updated.name}.`, {
        worker: updated,
      });
    },
  } as any);
  ...
  pi.registerTool({
    ...
    description:
      "Update a durable team task. Requires expectedVersion for CAS safety.",
    parameters: TeamTaskUpdateParams,
    async execute(_id, raw, _signal, _update, ctx) {
      const params = raw as { id: string; expectedVersion: number } & TaskPatch;
      const team = await activeTeam(ctx);
      try {
        const task = await state.store.updateTask(
          team.id,
          params.id,
          params,
        ...
        await updateWidget(ctx, state, team.id);
        return textResult(`Task updated: ${task.id} v${task.version}`, {
          task,
        });
      } catch (error) {
        if (error instanceof VersionConflictError) {
          return textResult(
            `Version conflict: task is v${error.actual}, retry with a fresh team_status.`,
            {
              conflict: true,
              actual: error.actual,
            },
  ...
  pi.registerTool({
    ...
    async execute(_id, raw, _signal, _update, ctx) {
      ...
      await updateWidget(ctx, state, team.id);
      return textResult(
        `Team control: ${params.action}${params.target ? ` ${params.target}` : ""}`,
      );
    },
  } as any);
  ...
  pi.registerCommand("team", {
    description:
      "Open Team Mode dashboard or run /team start|status|worker|tasks|pause|resume|stop|clear",
    handler: async (args, ctx) => {
      const [sub = "dashboard", ...rest] = args
        .trim()
        .split(/\s+/)
        .filter(Boolean);
      if (sub === "start") {
        ...
        if (!objective)
          return ctx.ui.notify("Usage: /team start <objective>", "warning");
      ...
      if (sub === "status")
        return ctx.ui.notify(
          formatStatus(await state.store.summary((await activeTeam(ctx)).id)),
          "info",
        );
      if (sub === "tasks")
        return ctx.ui.notify(
          formatTasks(
            (await state.store.summary((await activeTeam(ctx)).id))?.tasks ??
              [],
          ),
          "info",
        );
...
async function handleRuntimeEvent(
  ...
  const eventType =
    status === "completed" ? "worker_completed" : "worker_failed";
  if (worker.currentTaskId) {
    const task = (await state.store.listTasks(teamId)).find(
      (item) => item.id === worker.currentTaskId,
    );
    if (task && task.status !== "completed" && task.status !== "cancelled") {
      await state.store.updateTask(
        teamId,
        task.id,
        {
  ...
  const event = await state.store.appendEvent(teamId, {
    type: eventType,
    actor: worker.name,
  ...
  pi.sendMessage(
    {
      ...
      content: formatTeamNotification({
        teamId: event.teamId,
        worker: worker?.name ?? event.actor,
        taskId: event.taskId,
        status: event.type,
    ...
    },
    {
      triggerTurn:
        ...
        event.type === "attention_needed",
    },
  );
}

async function controlTeam(
  state: RuntimeState,
  team: TeamRecord,
  action: "pause" | "resume" | "stop",
  target?: string,
...
async function handleDashboardAction(
  ...
  if (action.type === "message") {
    ...
    if (message?.trim())
      await state.runtime.resumeWorker(
        action.worker,
        undefined,
        message.trim(),
      );
  ...
  if (action.type === "togglePause")
    await controlTeam(
      state,
      team,
      team.status === "paused" ? "resume" : "pause",
    );
  if (action.type === "stop")
    await controlTeam(state, team, "stop", action.worker?.name);
...
async function clearCurrentTeam(
  ...
  const team = await activeOrLatest(state);
  const activeWorkers = (await state.store.listWorkers(team.id)).filter(
    (worker) => worker.status === "running" || worker.status === "queued",
  );
  if (activeWorkers.length > 0)
    return ctx.ui.notify(
      "Cannot clear a team with active workers. Stop them first.",
      "warning",
    );
  const ok = await ctx.ui.confirm(
    "Clear team?",
    `Delete durable state for ${team.name}?`,
  );
  if (!ok) return;
...
function formatStatus(summary: TeamSummary | null): string {
  ...
  const running = summary.workers.filter(
...
function formatTasks(tasks: TeamTask[]): string {
  ...
  return tasks
    .map(
      (task) =>
        `• ${task.id} v${task.version} [${task.status}] ${task.subject}${task.owner ? ` — ${task.owner}` : ""}`,
    )
    .join("\n");
}
