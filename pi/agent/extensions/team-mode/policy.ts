...
const MUTATING_BASH =
  /\b(?:rm|mv|cp|chmod|chown|mkdir|touch|sed\s+-i|perl\s+-pi|git\s+(?:commit|reset|checkout|restore|clean|merge|rebase|apply|am)|npm\s+(?:install|update)|pnpm\s+(?:install|update)|bun\s+(?:add|remove))\b/;

export function defaultModeForRole(
  role: WorkerRole,
  requested?: WorkerMode,
): WorkerMode {
...
export function applyCoordinatorToolPolicy(pi: ExtensionAPI): void {
  ...
  const next = active.filter(
    (tool) => COORDINATOR_ALLOWED_TOOLS.has(tool) || tool.startsWith("team_"),
  );
  for (const tool of [
    "team_start",
    "team_spawn_worker",
  ...
  if (
    ...
    next.some((tool, index) => active[index] !== tool)
  ) {
    pi.setActiveTools(next);
  }
}

export function isBlockedCoordinatorTool(event: {
  toolName: string;
  input?: unknown;
}): string | undefined {
  if (
  ...
  if (event.toolName === "bash") {
    const command =
      typeof (event.input as { command?: unknown } | undefined)?.command ===
      "string"
