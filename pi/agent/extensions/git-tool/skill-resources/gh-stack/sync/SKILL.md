...
3. If sync reports a rebase conflict, it restores every branch to its original state — use the `restack` skill to resolve the conflicts interactively.
4. If sync reports that the local and remote stacks have diverged, it aborts without pushing anything. Report the divergence and let the user choose: adopt the remote stack, delete the stack on GitHub and recreate it with `submit`, or restructure locally first.
