	...
	for (const dir of candidateDirs(cwd)) {
		for (const filename of LOCAL_CONTEXT_FILENAMES) {
			...
			try {
				...
				console.error(`[agents-local] failed to read ${path}: ${error instanceof Error ? error.message : String(error)}`);
			}
			break;
		}
	}
...
export default function agentsLocalExtension(pi: ExtensionAPI) {
	...
	pi.registerCommand("agents-local", {
		...
		handler: async (_args, ctx) => {
			const files = loadLocalContextFiles(ctx.cwd);
			if (files.length === 0) {
				ctx.ui.notify(`No ${LOCAL_CONTEXT_FILENAMES.join(" or ")} files found from cwd to filesystem root.`, "info");
				return;
			}
