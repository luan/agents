...
function wrapProvider(
  ...
  if (typeof base.shouldTriggerFileCompletion === "function") {
    wrapped.shouldTriggerFileCompletion =
      base.shouldTriggerFileCompletion.bind(base);
  }
  return wrapped;
}
...
function patchDollarAutocompleteTrigger() {
  ...
  proto.handleInput = function (this: Record<string, unknown>, data: string) {
    ...
    const getCursor = this.getCursor as
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    ctx.ui.addAutocompleteProvider((current) =>
      wrapProvider(current, currentItems),
    );
  });
}
...
function colorizePlain(text: string, skills: Set<string>): string {
  return text
    .replace(DOLLAR_RE, (m, name: string) =>
      skills.has(name) ? HL(`$${name}`) : m,
    )
