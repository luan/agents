...
function rollbackFrontmatterWrites(writtenPaths: string[], originalContents: Map<string, string>): void {
...
export function applyChanges(
	...
	const existingDisableDirs = new Set(
		newSkills.filter((s) => s.startsWith("-")).map((s) => resolvePathFromBase(s.slice(1), settingsBaseDir)),
	);
