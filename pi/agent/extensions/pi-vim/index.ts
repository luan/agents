...
import { WordBoundaryCache, type WordMotionDirection, type WordMotionTarget } from "./word-boundary-cache.js";
...
function readClipboardInChildProcess(): string | null {
	try {
		const result = spawnSync(process.execPath, ["--input-type=module", "-e", CLIPBOARD_READ_HELPER_SOURCE], {
			encoding: "utf8",
...
function getAbortError(signal: AbortSignal): Error {
	return signal.reason instanceof Error ? signal.reason : createClipboardAbortError("clipboard write aborted");
}
...
export class ModalEditor extends CustomEditor {
	...
	private visualAnchorAbs: number | null = null;
	...
	private withTransition<T>(transition: Exclude<TransitionState, "none">, action: () => T): T {
		...
		for (let i = 0; i < maxSteps; i++) {
			...
			this.withTransition("redo", () => {
				...
				if (typeof editor.pushUndoSnapshot !== "function") {
					throw new Error("Redo restore prerequisite: pushUndoSnapshot unavailable");
				}
	...
	private clearVisualMode(): void {
		this.visualAnchorAbs = null;
		this.mode = "normal";
	...
	private applySyntheticEdit(mutation: () => void): void {
		...
		if (typeof editor.pushUndoSnapshot !== "function") {
			throw new Error("Synthetic edit prerequisite: pushUndoSnapshot unavailable");
		}
	...
	private clearPendingExCommand(): void {
		const shouldDiscardBracketedPasteTail =
			this.acceptingBracketedPasteInExCommand || this.pendingEscWhileAcceptingBracketedPasteInExCommand;
	...
	private handlePendingExCommandControlChunk(data: string): boolean {
		if (!data.includes("\r") && !data.includes("\n") && !data.includes("\x7f") && !data.includes("\x08")) {
			return false;
		}
	...
	private takeTotalCount(defaultValue: number = 1): number {
		...
		const total = prefix !== null && operator !== null ? prefix * operator : (prefix ?? operator ?? defaultValue);
	...
	private handlePendingDelete(data: string): void {
		...
		const supportsCountedWordMotion =
			data === "w" || data === "e" || data === "b" || data === "W" || data === "E" || data === "B";
	...
	private handlePendingChange(data: string): void {
		...
		const motionCount = supportsCountedWordMotion ? this.takeTotalCount(1) : 1;
		const effectiveMotion = data === "W" && this.isCursorOnNonWhitespace() ? "E" : data;
	...
	private enterVisualMode(): void {
		this.visualAnchorAbs = this.getAbsoluteIndexFromCursor();
		this.mode = "visual";
	}

	private codeUnitEndAbs(abs: number): number {
		const text = this.getText();
		return Math.min(text.length, abs + 1);
	}

	private getVisualRange(): { start: number; end: number } | null {
		if (this.visualAnchorAbs === null) return null;
		const cursorAbs = this.getAbsoluteIndexFromCursor();
		const anchorEnd = this.codeUnitEndAbs(this.visualAnchorAbs);
		const cursorEnd = this.codeUnitEndAbs(cursorAbs);
		const start = Math.min(this.visualAnchorAbs, cursorAbs);
	...
	private handleVisualMode(data: string): void {
		...
		if (data === "y") {
			...
			if (range) this.writeToRegister(this.getText().slice(range.start, range.end), "yank");
			this.clearVisualMode();
			return;
		}
		...
		if (data === "d" || data === "x") {
			const range = this.getVisualRange();
			if (range) {
				this.writeToRegister(this.getText().slice(range.start, range.end));
				this.replaceTextInBuffer(
					this.getText().slice(0, range.start) + this.getText().slice(range.end),
					range.start,
				);
			}
			this.clearVisualMode();
		...
		const anchor = this.visualAnchorAbs;
		this.handleNormalMode(data);
		this.visualAnchorAbs = anchor;
		this.mode = "visual";
	}
	...
	private handleNormalMode(data: string): void {
		...
		if (this.prefixCount.length > 0) {
			...
			const supportsCountedCharMotion = CHAR_MOTION_KEYS.has(data) || data === ";" || data === ",";
			const supportsCountedWordMotion =
				data === "w" || data === "e" || data === "b" || data === "W" || data === "E" || data === "B";
			const supportsCountedParagraphMotion = data === "{" || data === "}";
			const supportsCountedNav = data === "h" || data === "j" || data === "k" || data === "l";
	...
	private charType(ch: string | undefined, semanticClass: WordMotionClass = "word"): "space" | "word" | "other" {
	...
	private resolveWordMotion(motion: string): { motion: "w" | "e" | "b"; semanticClass: WordMotionClass } | null {
	...
	private tryFindWordTargetInLine(
		...
		const targetCol = this.wordBoundaryCache.tryFindTarget(line, col, direction, target, semanticClass);
	...
	private tryFindWordTargetLineLocal(
		...
		const targetCol = this.tryFindWordTargetInLine(lineSnapshot, col, direction, target, false, semanticClass);
		if (targetCol === null) return null;
	...
	private moveWord(
		...
		while (remaining > 0) {
			...
			const targetAbs = this.findWordTargetInText(text, currentAbs, direction, target, remaining, semanticClass);
	...
	private deleteWithMotion(motion: string, count: number = 1): boolean {
		...
		if (wordMotion) {
			const lineLocalRange = this.tryWordMotionLineLocalRange(wordMotion.motion, count, wordMotion.semanticClass);
			if (lineLocalRange) {
				this.deleteRange(lineLocalRange.col, lineLocalRange.targetCol, lineLocalRange.inclusive);
				return true;
			}
	...
	private yankWithMotion(motion: string): boolean {
		...
		const wordMotion = this.resolveWordMotion(motion);
		if (wordMotion) {
			const lineLocalRange = this.tryWordMotionLineLocalRange(wordMotion.motion, 1, wordMotion.semanticClass);
			if (lineLocalRange) {
				this.yankRange(lineLocalRange.col, lineLocalRange.targetCol, lineLocalRange.inclusive);
				return true;
			}
	...
	private getWordObjectRange(
		...
		return resolveWordTextObjectRange(line, lineStartAbs, cursor.col, kind, count, semanticClass);
	}
	...
	private highlightRenderedSelectionLine(
		...
		for (let i = 0; i < line.length; i++) {
			...
			if (char === "\x1b") {
				const next = line[i + 1];
				const end = next === "]" || next === "_" ? line.indexOf("\x07", i + 2) : line.slice(i).search(/[A-Za-z~]/);
				if (end === -1) {
					result += line.slice(i);
					break;
				}
				const endIndex = next === "]" || next === "_" ? end : i + end;
				result += line.slice(i, endIndex + 1);
				i = endIndex;
				continue;
			}

			while (sourceText[sourceAbs.value] === "\n") sourceAbs.value++;
			if (sourceText[sourceAbs.value] === char) {
				result += sourceAbs.value >= range.start && sourceAbs.value < range.end ? colorize(char) : char;
				sourceAbs.value++;
				continue;
			}
	...
	private applyVisualSelectionHighlight(lines: string[]): void {
		if (this.mode !== "visual") return;
		const range = this.getVisualRange();
		const colorize = this.labelColorizers?.visual;
		if (!range || !colorize) return;

		const sourceText = this.getText();
		const sourceAbs = { value: 0 };
		for (let i = 0; i < lines.length; i++) {
			lines[i] = this.highlightRenderedSelectionLine(lines[i] ?? "", sourceText, range, sourceAbs, colorize);
		}
	}
	...
	private getModeLabelColorizer(): ((s: string) => string) | null {
		...
		if ((this.mode as string) === "visual") return this.labelColorizers.visual;
	...
	private getModeLabel(): string {
		...
		if (this.pendingG) {
			return this.pendingGCount ? ` NORMAL g${this.pendingGCount}_ ` : " NORMAL g_ ";
		}
