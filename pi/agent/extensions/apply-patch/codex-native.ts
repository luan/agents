...
function isFunctionToolNamed(tool: unknown, name: string): tool is FunctionToolPayload {
	return !!tool && typeof tool === "object" && (tool as FunctionToolPayload).type === "function" && (tool as FunctionToolPayload).name === name;
}
...
export function getOpenAICodexImagePath(cwd: string, responseId: string | undefined, callId: string, outputFormat?: string): string {
	...
	return resolve(getOpenAICodexImageDirectory(cwd), `${shortenFilePart(callId, "image")}-${shortenFilePart(responseId, "response")}.${ext}`);
}
...
export function extractWebSearch(item: unknown): SurfacedWebSearch | undefined {
	...
	const callId = typeof record.id === "string" ? record.id : typeof record.call_id === "string" ? record.call_id : undefined;
	if (!callId) return undefined;

	const action = typeof record.action === "object" && record.action !== null ? (record.action as Record<string, unknown>) : {};
	const query = typeof action.query === "string" ? action.query : typeof record.query === "string" ? record.query : undefined;
	...
	const addSource = (source: unknown) => {
		...
		seen.add(url);
		sources.push({ url, ...(typeof sourceRecord.title === "string" ? { title: sourceRecord.title } : {}) });
	};
	...
	return searches
		.map((search, index) => {
			...
			if (search.sources.length > 0) {
				lines.push("Sources:");
				for (const source of search.sources.slice(0, 5)) lines.push(`- ${source.title ? `${source.title} — ` : ""}${source.url}`);
			}
			return lines.join("\n");
		})
		.join("\n\n");
...
export function createImageGenerationTool(): ToolDefinition<any> {
	return {
		...
		promptSnippet:
			"Generate an image with native OpenAI Codex image_generation. Outputs are saved under `.pi/openai-codex-images/` and mirrored to `latest.png`.",
		parameters: Type.Unsafe<Record<string, never>>({ type: "object", additionalProperties: false }),
		prepareArguments: () => ({}),
...
export function renderImageGenerationMessage(message: { content: unknown; details?: ImageDisplayMessageDetails }, options: { expanded?: boolean }, theme: any) {
	...
	if (savedImage) {
		...
		try {
			...
			box.addChild(new Image(data, `image/${savedImage.outputFormat}`, { fallbackColor: (text: string) => theme.fg("customMessageText", text) }, { maxWidthCells: 60 }));
		} catch {
...
export function renderWebSearchMessage(message: { content: unknown; details?: { searches?: SurfacedWebSearch[] } }, options: { expanded?: boolean }, theme: any) {
	const box = new Box(1, 1, (text) => theme.bg("customMessageBg", text));
