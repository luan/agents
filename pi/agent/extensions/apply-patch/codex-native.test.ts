...
test("rewrites image_generation only for image-capable openai-codex models", () => {
	...
	expect(rewritten.tools[0]).toEqual({ type: "image_generation", output_format: "png" });

	const textOnly = rewriteNativeImageGenerationTool(payload, { ...codexModel, input: ["text"] } as never) as typeof payload;
