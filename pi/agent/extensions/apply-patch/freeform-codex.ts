  ...
  const flushPendingMessages = () => {
    ...
    for (let index = 0; index < activities.length; index++) {
      ...
      while (
        ...
        activities[index + 1]?.kind === "web-search"
      ) {
        searches.push(
          (
            activities[++index] as Extract<
              PendingActivity,
              { kind: "web-search" }
            >
          ).search,
      ...
      await processResponsesStream(
        mapFreeformEvents(
          captureNativeActivities(mapCodexEvents(parseSSE(response)), {
            ...
            requestPrompt,
            onImageSaved: (savedImage, imageData) =>
              deps.onImageSaved?.(savedImage, imageData),
...
function buildRequestBody(
  ...
  if (context.tools?.length)
    body.tools = convertTools(context.tools, applyPatch);
...
function convertFreeformResponsesMessages(
  ...
  return messages.map((item: any) => {
    ...
    if (
      ...
      applyPatchCallIds.has(item.call_id)
    ) {
      return {
...
export async function* mapFreeformEvents(
...
async function* captureNativeActivities(
  ...
  for await (const event of events) {
    ...
    if (
      ...
      const callId =
        typeof event.item.id === "string" ? event.item.id : undefined;
      const result =
        typeof event.item.result === "string" ? event.item.result : undefined;
      if (callId && result) {
        const outputFormat =
          typeof event.item.output_format === "string"
