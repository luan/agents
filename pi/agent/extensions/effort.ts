...
export default function effortExtension(pi: ExtensionAPI) {
  ...
  pi.registerCommand("effort", {
    description:
      "Set thinking effort for the current model and persist to effort.json",
    ...
    handler: async (args, ctx) => {
      ...
      if (!VALID.has(arg as Level)) {
        ctx.ui.notify(
          `Invalid level "${arg}". Valid: ${LEVELS.join(", ")}`,
          "error",
        );
        return;
      }
      ...
      try {
        ...
        ctx.ui.notify(
          `Failed to save effort.json: ${err instanceof Error ? err.message : String(err)}`,
          "error",
        );
        return;
      }
