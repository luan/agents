...
const sessionTurnSchema = {
  session: Type.Optional(
    Type.String({ description: "Session id; defaults to current Pi session." }),
  ),
  turn: Type.Optional(
    Type.String({ description: "Turn id; defaults to current Pi turn." }),
  ),
};
...
const diagnosticsSchema = Type.Object({
  ...
  all: Type.Optional(
    Type.Boolean({ description: "Show all diagnostics for list." }),
  ),
  source: Type.Optional(
    Type.String({ description: "Diagnostic source for record." }),
  ),
  scopeKind: Type.Optional(
    Type.String({ description: "Diagnostic scope kind." }),
  ),
  scopeKey: Type.Optional(
    Type.String({ description: "Diagnostic scope key." }),
  ),
  severity: Type.Optional(
    StringEnum(["error", "warning", "info", "hint"] as const),
  ),
  code: Type.Optional(Type.String({ description: "Diagnostic code." })),
  message: Type.Optional(
    Type.String({ description: "Diagnostic message for record." }),
  ),
  ...
  fingerprint: Type.Optional(
    Type.String({ description: "Stable diagnostic fingerprint." }),
  ),
  snapshot: Type.Optional(
    Type.Any({ description: "DiagnosticSnapshotInput JSON for snapshot." }),
  ),
});

const checksSchema = Type.Object({
  action: Type.Optional(StringEnum(["list", "run"] as const)),
  automatic: Type.Optional(
    Type.Boolean({ description: "Run only automatic checks/scanners." }),
  ),
  all: Type.Optional(
    Type.Boolean({ description: "Run all configured checks/scanners." }),
  ),
  name: Type.Optional(
    Type.Array(
      Type.String({ description: "Configured check/scanner name to run." }),
    ),
  ),
  scanners: Type.Optional(
    Type.Boolean({ description: "Include configured scanners." }),
  ),
});
...
const healthSchema = Type.Object({
  ...
  finalOutput: Type.Optional(
    Type.Boolean({
      description: "Request final/agent-end style text for CLI parity.",
    }),
  ),
});
...
const cleanupSchema = Type.Object({
  ...
  allowUnsafe: Type.Optional(
    Type.Boolean({
      description: "Run unsafe/invasive registry entries explicitly.",
    }),
  ),
});
...
const reportSchema = Type.Object({
  ...
  turn: sessionTurnSchema.turn,
  path: Type.Optional(
    Type.String({ description: "Optional changed-file path." }),
  ),
});
...
const contextSchema = Type.Object({
  ...
  ack: Type.Optional(
    Type.Boolean({
      description: "Acknowledge current warning-or-worse health.",
    }),
  ),
});
...
const rawOutputSchema = Type.Object({
  ...
  limit: Type.Optional(
    Type.Number({ description: "Maximum outputs to list." }),
  ),
});
...
const pruneSchema = Type.Object({
  dryRun: Type.Optional(
    Type.Boolean({
      description: "Preview retention pruning without deleting.",
    }),
  ),
});
...
const lensToolNames = new Set<string>(LENS_TOOL_NAMES);
...
export function shouldApplyHookUiForTool(
  toolName: string | undefined,
): boolean {
  return !toolName || !lensToolNames.has(toolName);
}
...
export function applyLensUi(ctx: any, response: unknown) {
  try {
    ...
    ctx.ui.setWidget(
      "lens-health",
      renderLensWidgetLines(response, false, { ansi: true }),
    );
  } catch (error) {
...
function applyLensToolResultUi(ctx: any, result: unknown) {
  const envelope = (result as any)?.details?.results ?? result;
  applyLensUi(ctx, envelope);
}

async function runCtLens(
  args: string[],
  cwd: string,
  signal?: AbortSignal,
  input?: string,
  ...
  const result = await runCommand("ct", fullArgs, cwd, {
    signal,
    input,
    allowNonZero: true,
  });
  ...
  try {
    ...
    if (result.exitCode !== 0)
      throw new Error(
        `${formatCommand("ct", fullArgs)} failed with exit code ${result.exitCode}${result.stderr.trim() ? `: ${result.stderr.trim()}` : ""}`,
      );
    throw error;
  }
  return {
    ...
    details: {
      command: formatCommand("ct", fullArgs),
      cwd,
      results: parsed,
...
export async function runLensHookCommand(
  ...
  try {
    ...
    return hookFailureResponse(
      "hook_command_failed",
      `ct hook failed to start: ${errorMessage(error)}`,
    );
  }
  ...
  try {
    ...
    return hookFailureResponse(
      ...
      hookFailureMessage(error, result),
      result,
    );
  }
}
...
export default function lensExtension(pi: ExtensionAPI) {
  ...
  async function runCtLensTool(
    ...
    const result = await runCtLens(args, cwd, signal, input);
    applyLensToolResultUi(ctx, result);
    return result;
  }

  function eventFor(
    ctx: any,
    event: LensHookEventName,
  ...
  async function ignoreStaleCtx<T>(
    task: () => Promise<T>,
  ): Promise<T | undefined> {
    try {
      return await task();
  ...
  pi.on("turn_start", async (event, ctx) => {
    await ignoreStaleCtx(async () => {
      activeTurnIndex = Number.isFinite(event.turnIndex)
        ? event.turnIndex
        : activeTurnIndex;
      activeTurnId = `turn-${agentSeq}-${activeTurnIndex}`;
      const response = await runHook(
        "lens-turn-start",
        eventFor(ctx, "turn_start"),
        safeCwd(ctx),
        safeSignal(ctx),
      );
      applyLensUi(ctx, response);
  ...
  pi.on("tool_call", async (event, ctx) => {
    await ignoreStaleCtx(async () => {
      ...
      if (shouldApplyHookUiForTool(event.toolName)) applyLensUi(ctx, response);
    });
  });
    ...
    await ignoreStaleCtx(async () => {
      const response = await runHook(
        ...
        eventFor(ctx, "post_tool", {
          ...
          known_files: filesFromToolAndResult(
            event.toolName,
            event.input,
            event.details,
          ),
  ...
  registerTool({
    ...
    description:
      "List, record, or snapshot Lens diagnostics via ct lens diagnostics. Full JSON is preserved in details.",
    ...
    async execute(_id, params, signal, _onUpdate, ctx) {
      ...
      if (action === "record") {
        const args = [
          ...
          "record",
          "--source",
          required(params.source, "source"),
          "--severity",
          required(params.severity, "severity"),
          "--message",
        ...
        return runCtLensTool(args, ctx.cwd, signal, ctx);
      }
      if (action === "snapshot") {
        return runCtLensTool(
          ["diagnostics", "snapshot"],
          ctx.cwd,
          signal,
          ctx,
          JSON.stringify(required(params.snapshot, "snapshot")),
        );
      }
    ...
    renderCall(args, theme, ctx) {
      return renderText(
        ctx,
        title(theme, nf.diagnostics, "lens diagnostics", args.action ?? "list"),
      );
    },
    renderResult(result, options, _theme, ctx) {
      return renderText(
        ctx,
        summarizeLensResult(result, options.expanded, { ansi: true }),
      );
    },
  });
  ...
  registerTool({
    ...
    label: "Lens checks",
    description:
      "List or run repository-configured Lens checks and scanners via ct lens checks.",
    ...
    renderCall(args, theme, ctx) {
      return renderText(
        ctx,
        title(theme, nf.lens, "lens checks", args.action ?? "list"),
      );
    },
    renderResult(result, options, _theme, ctx) {
      return renderText(
  ...
  registerTool({
    ...
    async execute(_id, _params, signal, _onUpdate, ctx) {
      return runCtLensTool(["status"], ctx.cwd, signal, ctx);
    },
  ...
  registerTool({
    ...
    async execute(_id, params, signal, _onUpdate, ctx) {
      const args = [
        "health",
        "--session",
        params.session ?? currentSession(ctx),
        "--turn",
        params.turn ?? currentTurn(),
      ];
    ...
    renderCall(args, theme, ctx) {
      return renderText(
        ...
        title(
          ...
          nf.lens,
          "lens cleanup",
          args.allowUnsafe ? "allow unsafe" : "safe",
        ),
      );
    },
    renderResult(result, options, _theme, ctx) {
  ...
  registerTool({
    ...
    label: "Lens report",
    description:
      "Show a deeper Lens changed-file report with diagnostics, cleanup, patch refs, and symbols.",
    ...
    renderCall(args, theme, ctx) {
      return renderText(
        ctx,
        title(theme, nf.files, "lens report", args.path ?? "turn"),
      );
    },
    renderResult(result, options, _theme, ctx) {
      return renderText(
  ...
  registerTool({
    ...
    renderCall(args, theme, ctx) {
      return renderText(
        ctx,
        title(theme, nf.lens, "lens context", args.ack ? "ack" : "show"),
      );
    },
    renderResult(result, options, _theme, ctx) {
      return renderText(
  ...
  registerTool({
    ...
    description:
      "List or show retained sanitized Lens raw output via ct lens raw-output.",
    ...
    renderCall(args, theme, ctx) {
      return renderText(
        ctx,
        title(theme, nf.lens, "lens raw output", args.action ?? "list"),
      );
    },
    renderResult(result, options, _theme, ctx) {
      return renderText(
  ...
  registerTool({
    ...
    label: "Lens prune",
    description:
      "Prune Lens telemetry using retention policy via ct lens prune.",
  ...
  for (const key of ["paths", "files"]) {
    ...
    for (const item of input[key])
      if (typeof item === "string")
        files.push(file(item, readLikeOperation(toolName)));
  }
  return files;
}

export function filesFromToolAndResult(
  toolName: string,
  input: any,
  details: any,
): ToolFile[] {
  return dedupeFiles([
    ...filesFromTool(toolName, input),
    ...filesFromToolResult(toolName, details),
  ]);
}
...
function filesFromApplyPatchInput(input: any): ToolFile[] {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    const update = line.match(
      /^\*\*\* (?:Update File|Update Scope|Replace All In File): (.+)$/,
    );
    if (update) {
...
function file(
  ...
  if (typeof startLine === "number" && Number.isFinite(startLine))
    out.start_line = Math.max(1, Math.trunc(startLine));
