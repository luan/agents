	...
	try {
		...
		for (const entry of entries) {
			...
			if (isDirectory) {
				...
				const isRootMd = (includeRootFiles ?? false) && entry.name.endsWith(".md");
...
function scanSkillPath(sourcePath: string, skills: RawSkill[], visitedRealPaths: Set<string>): void {
...
function getDisabledPaths(settings: Settings, settingsBaseDir: string): Set<string> {
...
function parseGitSource(source: string): { host: string; repoPath: string } | null {
	...
	if (withoutRef.startsWith("http://") || withoutRef.startsWith("https://") || withoutRef.startsWith("ssh://")) {
		try {
...
function resolvePackageRoot(source: string, settingsBaseDir: string, npmRoot: string | null): string | null {
	const trimmed = source.trim();
...
function collectConfiguredSkillPaths(settings: Settings, settingsBaseDir: string): string[] {
...
export function loadAllSkills(
	...
	const configuredPaths = collectConfiguredSkillPaths(settings, resolvedSettingsBaseDir);

	const scanTargets = uniquePaths([...(overrideDirs ?? defaultScanDirs), ...configuredPaths]);
	...
	for (const [name, skill] of byName) {
		...
		const allDisabled = allPaths.every((p) => isSkillDisabled(path.resolve(p), disabledPaths));
