...
const commonOptions = {
  db: Type.Optional(
    Type.String({ description: "Override source database path" }),
  ),
};
...
const sourceShowTargetList = Type.Array(
  Type.String({ description: "Symbol target" }),
  {
...
const sourceSearchSchema = Type.Object({
  query: Type.Optional(
    Type.String({
      description: "Search query; structural mode may use pattern instead",
    }),
  ),
  mode: Type.Optional(
    Type.Union(
  ...
  pattern: Type.Optional(
    Type.String({ description: "Structural AST pattern; defaults to query" }),
  ),
  limit: Type.Optional(
  ...
  lang: Type.Optional(
    Type.String({
      description: "Language filter; required for structural mode",
    }),
  ),
  exact: Type.Optional(
    Type.Boolean({ description: "Require exact matching where supported" }),
  ),
  ignoreCase: Type.Optional(
    Type.Boolean({ description: "Case-insensitive matching where supported" }),
  ),
  paths: Type.Optional(
    Type.Array(Type.String({ description: "Include globs" })),
  ),
  excludes: Type.Optional(
    Type.Array(Type.String({ description: "Exclude globs" })),
  ),
  selector: Type.Optional(
    Type.String({ description: "ast-grep selector for structural mode" }),
  ),
  context: Type.Optional(
    Type.Number({ description: "Context lines for structural matches" }),
  ),
  includeIgnored: Type.Optional(
    Type.Boolean({ description: "Include ignored files where supported" }),
  ),
  ...commonOptions,
});
...
const sourceShowSchema = Type.Object({
  ...
  all: Type.Optional(
    Type.Boolean({
      description: "Return every definition for ambiguous targets",
    }),
  ),
  ...commonOptions,
});
...
const sourceOutlineSchema = Type.Object({
  ...
  signatures: Type.Optional(
    Type.Boolean({ description: "Include signatures" }),
  ),
...
const sourceRefsSchema = Type.Object({
  targets: targetList,
  importers: Type.Optional(
    Type.Boolean({ description: "Also include importers" }),
  ),
  impact: Type.Optional(
    Type.Boolean({ description: "Include shallow impact" }),
  ),
  depth: Type.Optional(
    Type.Number({ description: "Reference/importer depth" }),
  ),
  limit: Type.Optional(Type.Number({ description: "Maximum results" })),
  context: Type.Optional(
    Type.Number({ description: "Context lines around references" }),
  ),
  paths: Type.Optional(
    Type.Array(Type.String({ description: "Include globs" })),
  ),
  ...
  file: Type.Optional(
    Type.String({
      description: "Limit references to paths containing this fragment",
    }),
  ),
  ...commonOptions,
});
...
const sourceGraphSchema = Type.Object({
  ...
  limit: Type.Optional(Type.Number({ description: "Maximum results" })),
  context: Type.Optional(
    Type.Number({ description: "Context lines around hits; impact only" }),
  ),
  kinds: Type.Optional(
    Type.String({ description: "Trace edge kinds, e.g. call or call,use" }),
  ),
  ...commonOptions,
});
...
const sourceImplsSchema = Type.Object({
  ...
  of: Type.Optional(
    Type.String({
      description: "Find protocols/interfaces implemented by this symbol",
    }),
  ),
  resolved: Type.Optional(
    Type.Boolean({
      description: "Only include resolved implementation targets",
    }),
  ),
  unresolved: Type.Optional(
    Type.Boolean({
      description: "Only include unresolved implementation targets",
    }),
  ),
  ...commonOptions,
});
...
const sourceDiffSchema = Type.Object({
  target: Type.String({
    description: "Symbol whose definition-scoped diff should be shown",
  }),
  base: Type.Optional(Type.String({ description: "Base ref" })),
  stat: Type.Optional(
    Type.Boolean({ description: "Return diffstat instead of full diff" }),
  ),
  ...commonOptions,
});
...
function summarize(
  ...
  const prefix = okLine(theme, [
    chip(theme, "󰓹", details.operation, count ?? "ok"),
  ]);
  ...
  if (Array.isArray(results)) {
    for (const item of results.slice(0, 20)) {
      if (item && typeof item === "object") {
        ...
        const loc = path
          ? theme.fg(
              "dim",
              ` ${path}${anyItem.start_line ? `:${anyItem.start_line}` : ""}`,
            )
          : "";
    ...
    if (results.length > 20)
      lines.push(theme.fg("muted", `  … ${results.length - 20} more`));
    return lines.join("\n");
  }
...
function renderSourceCall(
  ...
  return renderText(
    ctx,
    title(theme, "󰓹", `source ${operation}`, target ? truncate(target) : ""),
  );
}
...
function renderResult(
  ...
  if (options.isPartial)
    return renderText(
      ctx,
      warnLine(theme, [chip(theme, "󰓹", "source", "running…")]),
    );
  return renderText(
    ctx,
    summarize(
      result.details as SourceDetails | undefined,
...
export default function sourceExtension(pi: ExtensionAPI) {
  ...
  registerTool({
    ...
    label: "source search",
    description:
      "Search source by symbol, text, path, or structural AST pattern.",
    promptSnippet:
      "Use source_search for code search. Modes: symbol, text, path, structural.",
    promptGuidelines: [
      "Use source_search instead of grep/find or backend-specific search tools.",
    ],
    ...
    renderCall: (args, theme, ctx) =>
      renderText(
        ...
        title(
          ...
          "󰓹",
          "source search",
          args.query ?? args.pattern ?? args.mode ?? "symbol",
        ),
      ),
    renderResult,
  });
  ...
  registerTool({
    ...
    promptSnippet:
      "Use source_show to resolve symbols to structured metadata; use read for source lines.",
    promptGuidelines: [
      "Use source_show instead of backend-specific symbol display tools.",
    ],
    ...
    renderCall: (args, theme, ctx) =>
      renderSourceCall("show", args.targets?.join(", ") ?? "", theme, ctx),
    renderResult,
  });
  ...
  registerTool({
    ...
    promptSnippet:
      "Use source_outline before opening large or unfamiliar source files.",
    promptGuidelines: [
      "Use source_outline instead of backend-specific outline tools.",
    ],
    ...
    renderCall: (args, theme, ctx) =>
      renderSourceCall("outline", args.file, theme, ctx),
    renderResult,
  });
  ...
  registerTool({
    ...
    promptGuidelines: [
      "Use source_investigate instead of backend-specific investigation tools.",
    ],
    ...
    renderCall: (args, theme, ctx) =>
      renderSourceCall("investigate", args.targets?.join(", "), theme, ctx),
    renderResult,
  });
  ...
  registerTool({
    ...
    promptGuidelines: [
      "Use source_refs instead of backend-specific reference tools.",
    ],
    ...
    renderCall: (args, theme, ctx) =>
      renderSourceCall("refs", args.targets?.join(", "), theme, ctx),
    renderResult,
  });
  ...
  registerTool({
    ...
    promptGuidelines: [
      "Use source_impact instead of backend-specific impact tools.",
    ],
    ...
    renderCall: (args, theme, ctx) =>
      renderSourceCall("impact", args.targets?.join(", "), theme, ctx),
    renderResult,
  });
  ...
  registerTool({
    ...
    promptGuidelines: [
      "Use source_trace instead of backend-specific trace tools.",
    ],
    parameters: sourceGraphSchema,
    executionMode: "parallel",
    async execute(_id, params, signal, _onUpdate, ctx) {
    ...
    renderCall: (args, theme, ctx) =>
      renderSourceCall("trace", args.targets?.join(", "), theme, ctx),
    renderResult,
  });
  ...
  registerTool({
    ...
    promptSnippet:
      "Use source_impls to find implementations or conformances for a type/interface.",
    promptGuidelines: [
      "Use source_impls instead of backend-specific implementation tools.",
    ],
    ...
    renderCall: (args, theme, ctx) =>
      renderSourceCall("impls", args.targets?.join(", "), theme, ctx),
    renderResult,
  });
  ...
  registerTool({
    ...
    promptSnippet:
      "Use source_diff to show git diff scoped to a symbol definition.",
    promptGuidelines: [
      "Use source_diff instead of backend-specific symbol diff tools.",
    ],
    ...
    renderCall: (args, theme, ctx) =>
      renderSourceCall("diff", args.target, theme, ctx),
    renderResult,
  });
}
