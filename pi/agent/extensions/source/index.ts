...
const sourceSearchSchema = Type.Object({
	...
	mode: Type.Optional(
		Type.Union([Type.Literal("symbol"), Type.Literal("text"), Type.Literal("path"), Type.Literal("structural")], {
...
export default function sourceExtension(pi: ExtensionAPI) {
	...
	registerTool({
		...
		renderCall: (args, theme, ctx) =>
			renderText(ctx, title(theme, "󰓹", "source search", args.query ?? args.pattern ?? args.mode ?? "symbol")),
		renderResult,
	});
