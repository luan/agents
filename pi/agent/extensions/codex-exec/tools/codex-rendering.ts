...
function formatTruncatedAboveLine(
...
function renderExplorationText(
  ...
  for (const [index, line] of coalesceReadGroups(actionGroups)
...
function highlightShellCommand(command: string, theme: RenderTheme): string {
  ...
  return segments
    .map((segment) =>
      segment.role ? theme.fg(segment.role, segment.text) : segment.text,
    )
    .join("");
}
    ...
    if (actions.every((action) => action.kind === "read")) {
      ...
      if (lastRead) {
        ...
        const labels = reads.map((read) =>
          duplicateNames.has(read.name) ? read.path : read.name,
        );
        flattened.push({
          kind: "read",
          command: labels.join(" && "),
