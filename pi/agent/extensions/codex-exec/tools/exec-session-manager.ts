...
function shouldSyncBashEnv(
  ...
  return (
    ...
    isFishShell(requestedShell || process.env.SHELL)
  );
}
...
function stripTerminalControlSequences(
  ...
  return withoutOscAndDcs
    .replace(/\u001B\[[0-?]*[ -/]*[@-~]/g, (sequence) =>
      preserveSgr && sequence.endsWith("m") ? sequence : "",
    )
...
function normalizePipeOutput(text: string): string {
  return sanitizeBinaryOutput(
    stripTerminalControlSequences(text, false, true),
    false,
    true,
  )
    .replace(/\r\n/g, "\n")
...
function writeTerminalChar(session: PtyExecSession, char: string): void {
  if (session.terminalCursor > session.terminalLine.length) {
    session.terminalLine.push(
      ...Array.from(
        { length: session.terminalCursor - session.terminalLine.length },
        () => " ",
      ),
    );
  }
...
function applyTerminalOutput(session: PtyExecSession, text: string): string {
  const sanitized =
    session.terminalPendingEscape + stripTerminalControlSequences(text, true);
  ...
  for (let index = 0; index < sanitized.length; index += 1) {
    ...
    if (char === "\u001b") {
      if (sanitized[index + 1] === "[") {
        ...
        if (finalByte === "m") {
          session.terminalStyle =
            params === "" || params.split(";").includes("0")
              ? ""
              : `\u001b[${params}m`;
        } else if (finalByte === "K") {
          const mode = Number(params || "0");
          if (mode === 0) {
            session.terminalLine = session.terminalLine.slice(
              0,
              session.terminalCursor,
            );
          } else if (mode === 1) {
...
export function createExecSessionManager(
  ...
  const minNonInteractiveExecYieldTimeMs = Math.min(
    ...
    Math.max(
      ...
      options.minNonInteractiveExecYieldTimeMs ??
        MIN_NON_INTERACTIVE_EXEC_YIELD_TIME_MS,
    ),
  );
  const minEmptyWriteYieldTimeMs = Math.min(
    ...
    Math.max(
      MIN_YIELD_TIME_MS,
      options.minEmptyWriteYieldTimeMs ?? MIN_EMPTY_WRITE_YIELD_TIME_MS,
    ),
  );
  const maxSessionBufferChars = Math.max(
    1024,
    options.maxSessionBufferChars ?? DEFAULT_MAX_SESSION_BUFFER_CHARS,
  );
