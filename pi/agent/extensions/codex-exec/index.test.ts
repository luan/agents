...
import { renderExecCommandCall, renderOutputBlock, renderWriteStdinCall, type RenderTheme } from "./tools/codex-rendering.ts";
...
test("exec command call renders Codex-style inline syntax-highlighted commands", () => {
	...
	expect(rendered.startsWith(`<success>•</success> <bold>Ran</bold> <syntaxFunction>git</syntaxFunction> diff <syntaxKeyword>--stat</syntaxKeyword>`)).toBe(true);
	expect(rendered).toContain(`<syntaxOperator>&&</syntaxOperator> <syntaxFunction>gh</syntaxFunction> pr edit 57220 <syntaxKeyword>--title</syntaxKeyword> <syntaxString>"fix(sync): resolve PBT convergence failures"</syntaxString>`);
...
test("exec command call unwraps simple shell wrappers before rendering", () => {
	...
	expect(rendered).toBe(`<dim>•</dim> <bold>Running</bold> <syntaxFunction>git</syntaxFunction> status <syntaxKeyword>--short</syntaxKeyword>`);
});
...
test("write stdin call uses unwrapped command previews", () => {
	...
	expect(rendered).toBe(`<success>• </success><bold>Waited for background terminal</bold><dim> · </dim><muted>git status --short</muted>`);
});
...
test("exec renderers self-render without the default success shell", () => {
	...
	try {
		registerExecCommandTool({ registerTool: (definition: any) => (tool = definition) } as any, createExecCommandTracker(), sessions);

		expect(tool.renderShell).toBe("self");
		const component = tool.renderResult(
			{ content: [{ type: "text", text: "fallback" }], details: { output: "visible output\nnext line\n", exit_code: 0 } },
...
test("write stdin renderer self-renders without the default success shell", () => {
	...
	try {
		...
		expect(tool.renderShell).toBe("self");
		const component = tool.renderResult(
			{ content: [{ type: "text", text: "" }], details: { output: "poll output\n", exit_code: 0 } },
			{ expanded: false, isPartial: false },
			testTheme,
		);
		const rendered = component.render(120).join("\n");
...
test("extension marks nonzero exec results as errors for red status dots", () => {
	...
	const nonzero = toolResultHandlers.map((handler) =>
		handler({ toolName: "exec_command", details: { output: "", exit_code: 1 } }),
	);
	const zero = toolResultHandlers.map((handler) => handler({ toolName: "exec_command", details: { output: "", exit_code: 0 } }));
...
test("exec session manager preserves ANSI SGR color output", async () => {
	const sessions = createExecSessionManager({ defaultExecYieldTimeMs: 5000 });
	try {
		const result = await sessions.exec({ cmd: "printf '\\033[32m✓ green\\033[0m\\n'", yield_time_ms: 5000 }, process.cwd());
