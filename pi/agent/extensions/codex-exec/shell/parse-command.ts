...
function summarizeMainTokens(mainCommand: string[]): ParsedShellCommand {
  ...
  if (head === "git") {
    ...
    if (subcommand === "ls-files") {
      const path = firstNonFlagOperand(subtail, [
        "--exclude",
        "--exclude-from",
        "--pathspec-from-file",
      ]);
...
function parseFindQueryAndPath(
  args: string[],
): [string | undefined, string | undefined] {
  const trimmed = trimAtConnector(args);
...
function isMutatingXargsCommand(tokens: string[]): boolean {
  return xargsSubcommand(tokens)?.length
    ? xargsIsMutatingSubcommand(xargsSubcommand(tokens)!)
    : false;
}
