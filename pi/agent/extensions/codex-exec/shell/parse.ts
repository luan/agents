...
export function parseShellPart(
  tokens: string[],
  cwd?: string,
): ShellAction | null {
...
function parseFdQueryAndPath(
  ...
  if (nonFlags.length === 1) {
    if (isPathish(nonFlags[0]))
      return [undefined, shortDisplayPath(nonFlags[0])];
...
function sedReadPath(args: string[]): string | undefined {
  ...
  for (let index = 0; index < tokens.length; index++) {
    ...
    if (
      ...
      isValidSedRange(tokens[index + 1])
    ) {
      hasRangeScript = true;
    }
