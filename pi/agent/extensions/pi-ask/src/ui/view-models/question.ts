...
export function buildQuestionScreenModel(context: QuestionRenderContext): QuestionScreenModel {
	const rows = context.options.map((option, index) => buildOptionRowModel(context, option, index));
...
function buildQuestionNoteModel(context: QuestionRenderContext): QuestionNoteModel | undefined {
...
function buildOptionRowModel(context: QuestionRenderContext, option: AskDisplayOption, index: number): OptionRowModel {
	const { question, state } = context;
	const answer = getAnswer(state, question.id);
	const selected = index === state.activeOptionIndex;
	const answered = option.isCustomOption ? !!answer?.customText : isOptionSelected(answer, option.value);
	return {
		color: getOptionColor(answered, selected),
		description: option.description,
		detail: context.question.type === "preview" ? undefined : buildOptionDetailModel(context, option, selected),
		index,
...
function buildOptionDetailModel(
	...
	const { state, question } = context;
	const answer = getAnswer(state, question.id);
	const inputOpen = !!option.isCustomOption && selected && isInputOpenForQuestion(state, question.id);
	const noteOpen = !option.isCustomOption && isOptionNoteOpen(state, question.id, option.value);
	const customText = option.isCustomOption ? answer?.customText : undefined;
	const note = option.isCustomOption ? undefined : getOptionNote(state, question.id, option.value);
...
function getPreviewLayout(
	...
	return context.width >= UI_DIMENSIONS.previewWideMinWidth && context.options.length > 0 ? "wide" : "stacked";
}
...
function getOptionColor(answered: boolean, selected: boolean): "accent" | "text" | "success" {
	if (answered) {
		return "success";
	}
	if (selected) {
