...
import { type ReviewAnswer, shouldRenderAnswersIndividually, toReviewAnswer } from "../../state/result.ts";
...
export function buildReviewScreenModel(state: AskState, width: number): ReviewScreenModel {
	...
	return {
		...
		layout: shouldUseWideSubmitLayout(width, actionColumnWidth) ? "wide" : "stacked",
		questions: state.questions.map((question) =>
			toReviewQuestionModel(question.label, toReviewAnswer(question, state.answers[question.id], showAllNotes)),
		),
	};
}

function toReviewQuestionModel(label: string, answer: ReviewAnswer | undefined): ReviewQuestionModel {
	if (!answer) {
		return { label, unanswered: true };
	}

	return {
		answerText: shouldRenderAnswersIndividually(answer) ? undefined : answer.labels.join(", "),
...
function getSubmitActionColumnWidth(): number {
	return Math.max(...SUBMIT_CHOICES.map((choice, index) => visibleWidth(`❯ ${index + 1}. ${choice}`)));
}

function shouldUseWideSubmitLayout(width: number, actionColumnWidth: number): boolean {
	return (
		width >= UI_DIMENSIONS.submitWideMinWidth && width - actionColumnWidth - 2 >= UI_DIMENSIONS.submitMinReviewWidth
	);
}
