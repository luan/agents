...
function renderTabs(state: AskState, theme: Theme, width: number): string {
	...
	const availableTabWidth = Math.max(1, width - visibleWidth(TAB_PREFIX) - visibleWidth(TAB_SUFFIX));
	...
	const visibleTabs = tabs
		...
		.map((tab) => tab.render)
		.join(" ");
	return truncateToWidth(`${TAB_PREFIX[0]}${leftArrow}${visibleTabs}${rightArrow}`, width);
}
...
function getVisibleTabRange(
	...
	const totalWidth =
		widths.reduce((sum, width) => sum + width, 0) + Math.max(0, widths.length - 1) * TAB_SEPARATOR_WIDTH;
	...
	while (true) {
		const expansion = getExpandableTab(widths, range, availableWidth, usedWidth, preferRight);
		if (!expansion) {
			return range;
		}
...
function getExpandableTab(
	...
	usedWidth: number,
	preferRight: boolean,
): { direction: "left" | "right"; nextIndex: number; width: number } | undefined {
	for (const direction of getGrowthDirections(preferRight)) {
		...
		if (nextWidth === undefined || usedWidth + TAB_SEPARATOR_WIDTH + nextWidth > availableWidth) {
			continue;
		}
...
function getNextTabIndex(range: { start: number; end: number }, direction: "left" | "right"): number {
	return direction === "left" ? range.start - 1 : range.end + 1;
}

function applyTabGrowth(range: { start: number; end: number }, nextIndex: number, direction: "left" | "right") {
	if (direction === "left") {
