...
export function toggleSelection(answer: AskStateAnswer, option: AskDisplayOption, index: number): AskStateAnswer {
	const next = cloneAnswer(answer);
	const selectedIndex = next.selected.findIndex((selection) => selection.value === option.value);
...
export function setSingleSelection(answer: AskStateAnswer, option: AskDisplayOption, index: number): AskStateAnswer {
	return {
		...emptyAnswer(),
		note: answer.note,
		optionNotes: answer.optionNotes ? { ...answer.optionNotes } : undefined,
...
export function saveCustomText(answer: AskStateAnswer, rawValue: string, mode: "single" | "multi"): AskStateAnswer {
...
export function saveQuestionNote(answer: AskStateAnswer, rawValue: string): AskStateAnswer {
	const next = cloneAnswer(answer);
	if (rawValue.trim()) {
		next.note = rawValue;
		return next;
...
export function saveOptionNote(answer: AskStateAnswer, optionValue: string, rawValue: string): AskStateAnswer {
	const next = cloneAnswer(answer);
	...
	next.optionNotes = Object.keys(optionNotes).length > 0 ? optionNotes : undefined;
	return next;
}
...
export function isResultAnswerCommitted(answer: AskResultAnswer): boolean {
	return answer.values.length > 0 || answer.labels.length > 0 || answer.indices.length > 0 || !!answer.customText;
}
...
export function isOptionSelected(answer: AskStateAnswer | undefined, optionValue: string): boolean {
	return !!answer?.selected.some((selection) => selection.value === optionValue);
}
...
export function serializeAnswer(answer: AskStateAnswer): AskResultAnswer {
	...
	const selectedNotes = answer.optionNotes
		? Object.fromEntries(
				answer.selected
					.map((selection) => [selection.value, answer.optionNotes?.[selection.value]])
	...
	return {
		...
		customText: answer.customText,
		note: answer.note,
		optionNotes: selectedNotes && Object.keys(selectedNotes).length > 0 ? selectedNotes : undefined,
	};
}
...
export function getExtraOptionNotes(args: {
	...
	return Object.entries(args.answer.optionNotes ?? {})
		...
		.map(([value, note]) => {
			const option = args.questionOptions.find((candidate) => candidate.value === value);
