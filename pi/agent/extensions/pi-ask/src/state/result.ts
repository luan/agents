...
export function toAskResult(state: AskState): AskResult {
	const answers = Object.fromEntries(
		Object.entries(state.answers)
			.map(([questionId, answer]) => [questionId, serializeAnswer(answer)] as const)
			.filter(([, answer]) =>
				state.mode === "elaborate" ? isResultAnswerCommitted(answer) : !isResultAnswerEmpty(answer),
			),
	);
	...
	return {
		...
		answers,
		continuation: state.mode === "elaborate" ? serializeContinuation(state, answers) : undefined,
		elaboration: state.mode === "elaborate" ? serializeElaboration(state) : undefined,
	};
}
...
function createElaborationQuestionContext(question: AskState["questions"][number]) {
	return {
		id: question.id,
		label: question.label,
		prompt: question.prompt,
...
function toCommittedResultAnswer(answer: AskStateAnswer | undefined): AskResultAnswer | undefined {
...
export function toReviewAnswer(
	...
	if (!(hasCommittedAnswer || serialized.note) && extraOptionNotes.length === 0) {
		return;
	}

	return {
		...serialized,
		extraOptionNotes: extraOptionNotes.length > 0 ? extraOptionNotes : undefined,
	};
}
...
export function shouldRenderAnswersIndividually(answer: ReviewAnswer): boolean {
	...
	return answer.labels.length > 1 || Boolean(answer.optionNotes && Object.keys(answer.optionNotes).length > 0);
}
