...
export function reduceAskState(state: AskState, action: AskAction): AskState {
	switch (action.type) {
		...
		case "OPEN_OPTION_NOTE":
			return setView(state, optionNoteView(action.questionId, action.optionValue));
		case "CONFIRM":
...
export function enterQuestionNoteMode(state: AskState, questionId: string): AskState {
	return reduceAskState(state, { type: "OPEN_QUESTION_NOTE", questionId });
}

export function enterOptionNoteMode(state: AskState, questionId: string, optionValue: string): AskState {
...
export function submitCustomAnswer(state: AskState, rawValue: string): AskState {
...
function activateCurrentOption(state: AskState, trigger: "toggle" | "confirm" | "digit"): AskState {
	const question = getCurrentQuestion(state);
	...
	if (question.type === "multi") {
		...
		return updateAnswer(state, question.id, (answer) => toggleSelection(answer, option, state.activeOptionIndex));
	}

	const nextState = updateAnswer(state, question.id, (answer) => {
		if (trigger === "toggle") {
			const isSelected = answer.selected.some((selection) => selection.value === option.value);
			if (isSelected) {
				return {
					...answer,
					selected: [],
...
function saveInputValue(state: AskState, rawValue: string, submit: boolean): AskState {
	...
	const nextState = updateAnswer(exitEditingView(state), question.id, (answer) =>
		saveCustomText(answer, rawValue, question.type === "multi" ? "multi" : "single"),
	);
...
function saveNoteValue(state: AskState, rawValue: string): AskState {
	...
	const nextState = updateAnswer(exitEditingView(state), questionId, (answer) =>
		optionValue ? saveOptionNote(answer, optionValue, rawValue) : saveQuestionNote(answer, rawValue),
	);
	return nextState;
}
...
function updateAnswer(
	state: AskState,
	questionId: string,
	mutate: (answer: ReturnType<typeof emptyAnswer>) => ReturnType<typeof emptyAnswer>,
): AskState {
