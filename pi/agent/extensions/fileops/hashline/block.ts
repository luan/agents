...
function resolveDiagnosticBlock(resolver: BlockResolver, path: string, text: string, line: number): BlockSpan | null {
	try {
		const result = resolver({ path, text, line });
		return result !== null && !isBlockResolverFailure(result) ? result : null;
	} catch {
		return null;
	}
}
...
export interface ResolveBlockEditsOptions {
	 ...
	 * `PUT >N*:` lowerings (closer anchor or unresolvable block).
...
export function resolveBlockEdits(
	...
	for (const edit of edits) {
		...
		const outcome: BlockResolverResult = resolver
			? resolver({ path, text, line: edit.anchor.line })
			: { reason: "parser_unavailable" };
		const failure: BlockResolverFailure | undefined =
			outcome === null ? { reason: "no_block" } : isBlockResolverFailure(outcome) ? outcome : undefined;
		const span = failure === undefined ? outcome : null;
		if (span === null) {
			if (onUnresolved === "drop") continue;
			if (failure?.reason !== "no_block") {
				throw new Error(
					`line ${edit.lineNum}: ${blockResolverFailureMessage(edit.anchor.line, op, failure?.reason ?? "parser_unavailable")}`,
				);
			}
			// `PUT >N*:` never fails the patch — lower it to plain
			// `PUT >N:` with a warning instead. Two flavors:
		...
		if (op === "insert_after") {
			// Mirror the parser's `PUT >N:` lowering: one `after_anchor`
