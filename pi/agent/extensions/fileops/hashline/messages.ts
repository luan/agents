...
import type { BlockResolverFailureReason, BlockSpan } from "./types";
...
/**
 * `PUT >N:` body indented shallower than the anchor: the landing slid
...
/** Legacy session-chain replay warning retained by the local recovery adapter. */
...
/**
 * `PUT <1:`/`PUT >$:` applied despite a stale snapshot tag.
...
function describeBlockResolverFailure(reason: BlockResolverFailureReason): string {
	switch (reason) {
		...
		case "unsupported_language":
			return "the language is unsupported";
		case "no_block":
...
export function blockResolverFailureMessage(line: number, op: BlockOp, reason: BlockResolverFailureReason): string {
	...
	if (reason === "unsupported_language" || reason === "parser_unavailable") {
		return (
			`${forms.form(line)} needs a tree-sitter grammar to find where the block ends, and ` +
			`${describeBlockResolverFailure(reason)}. Use the concrete form ${forms.plain(line)} with the exact end line.`
		);
	}
	return (
		`${forms.form(line)} could not resolve safely because ${describeBlockResolverFailure(reason)}. ` +
		"Re-read the target range and use a concrete line edit if that landing was intended."
	);
}
