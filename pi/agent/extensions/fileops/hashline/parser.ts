...
/** Preserve caller assertions on range endpoints while keeping interior anchors bare. */
function expandRange(range: ParsedRange): Anchor[] {
	const anchors: Anchor[] = [];
	for (let line = range.start.line; line <= range.end.line; line++) {
		const endpoint = line === range.start.line ? range.start : line === range.end.line ? range.end : undefined;
		anchors.push(endpoint === undefined ? { line } : { ...endpoint, line });
	}
	return anchors;
}
...
export class Executor {
	...
	#pushDeleteRange(range: ParsedRange, lineNum: number): void {
		for (const anchor of expandRange(range)) this.#pushDelete(anchor, lineNum);
	}
