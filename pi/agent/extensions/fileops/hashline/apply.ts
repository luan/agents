...
import type { Anchor, ApplyResult, Clipboard, Cursor, Edit, SyntaxValidator } from "./types";
import { anchorHashMismatch } from "./verify-anchors";
...
function cursorsMatch(left: Cursor, right: Cursor): boolean {
	if (left.kind !== right.kind) return false;
	if (left.kind === "bof" || left.kind === "eof") return true;
	if (right.kind === "bof" || right.kind === "eof") return false;
	return left.anchor.line === right.anchor.line;
}

function insertionNeighbors(cursor: Cursor, lineCount: number): { above: number; below: number } {
	if (cursor.kind === "bof") return { above: -1, below: 0 };
	if (cursor.kind === "eof") return { above: lineCount - 1, below: lineCount };
	if (cursor.kind === "before_anchor") return { above: cursor.anchor.line - 2, below: cursor.anchor.line - 1 };
	return { above: cursor.anchor.line - 1, below: cursor.anchor.line };
}

function matchingPrefixLength(payload: readonly string[], fileLines: readonly string[], above: number): number {
	for (let count = Math.min(payload.length, above + 1); count >= 2; count--) {
		if (payload.slice(0, count).every((line, offset) => line === fileLines[above - count + 1 + offset])) return count;
	}
	return 0;
}

function matchingSuffixLength(payload: readonly string[], fileLines: readonly string[], below: number): number {
	for (let count = Math.min(payload.length, fileLines.length - below); count >= 2; count--) {
		if (payload.slice(-count).every((line, offset) => line === fileLines[below + offset])) return count;
	}
	return 0;
}

function dropPureInsertDuplicates(
	edits: readonly AppliedEdit[],
	fileLines: readonly string[],
	allowGeneric: boolean,
): { edits: AppliedEdit[]; warnings: string[] } {
	const normalized: AppliedEdit[] = [];
	const warnings: string[] = [];
	for (let index = 0; index < edits.length; index++) {
		const first = edits[index];
		if (first.kind !== "insert" || first.mode === "replacement") {
		...
		while (
			...
			edits[end].kind === "insert" &&
			(edits[end] as InsertEdit).mode !== "replacement" &&
			edits[end].lineNum === first.lineNum &&
			cursorsMatch((edits[end] as InsertEdit).cursor, first.cursor)
		) {
			end++;
		}
		if (end < edits.length && edits[end].kind === "delete" && edits[end].lineNum === first.lineNum) {
			normalized.push(...edits.slice(index, end));
			index = end - 1;
			continue;
		}
		const group = edits.slice(index, end) as InsertEdit[];
		const payload = group.map((edit) => edit.text);
		const { above, below } = insertionNeighbors(first.cursor, fileLines.length);
		let leading = allowGeneric ? matchingPrefixLength(payload, fileLines, above) : 0;
		if (
			leading === 0 &&
			above >= 0 &&
			payload[0] === fileLines[above] &&
			isStructuralCloserLine(payload[0]) &&
			canDropStructuralBoundary(payload, payload.slice(1))
		) {
			leading = 1;
		}
		const remaining = payload.slice(leading);
		let trailing = allowGeneric ? matchingSuffixLength(remaining, fileLines, below) : 0;
		if (
			trailing === 0 &&
			below < fileLines.length &&
			remaining.at(-1) === fileLines[below] &&
			isStructuralCloserLine(remaining.at(-1) ?? "") &&
			canDropStructuralBoundary(remaining, remaining.slice(0, -1))
		) {
			trailing = 1;
		}
		if (leading > 0 || trailing > 0) {
			warnings.push(
				`Auto-dropped ${leading + trailing} duplicate line(s) from insert at patch line ${first.lineNum}; adjacent file content already contains them.`,
			);
		}
		normalized.push(...group.slice(leading, group.length - trailing));
		index = end - 1;
	}
	return { edits: normalized, warnings };
}
...
function canDropStructuralBoundary(fullPayload: readonly string[], keptPayload: readonly string[]): boolean {
	const zero = { paren: 0, bracket: 0, brace: 0 };
	return (
		balanceEqual(computeDelimiterBalance(keptPayload), zero) &&
		!balanceEqual(computeDelimiterBalance(fullPayload), zero)
	);
}
...
//
// The body rows of a `PUT >N:` hunk carry an implicit depth claim:
...
// indentation names.
//
// Inward (block-lowered inserts only): `PUT >N*:` anchors on the
...
// named the opener and never saw the closer; a plain `PUT >M:` on a
...
interface AfterInsertGroup {
	...
	members: number[];
	/** First line of the resolved block when lowered from `PUT >N*:`. */
	blockStart?: number;
}
 ...
 * when the body is shallower, or — for `PUT >N*:` lowerings —
...
export interface ApplyEditsOptions {
	/** Clipboard register shared across sections in one patch. */
	clipboard?: Clipboard;
	/** Anonymous empty-register paste behavior. */
	onEmptyPaste?: "throw" | "drop";
	/** Target file path for syntax probes. */
	path?: string;
	/** Host parser used to veto delimiter repairs. */
	syntaxValidator?: SyntaxValidator;
	/** Drop matching runs of two or more adjacent lines from pure insert payload boundaries. */
...
export function applyEdits(text: string, edits: readonly Edit[], options: ApplyEditsOptions = {}): ApplyResult {
	...
	const hashError = anchorHashMismatch(text, edits);
	...
	let targetEdits = dropTrailingPhantomDeletes(
		appliedEdits.map((edit, index) => cloneAppliedEdit(edit, index)),
		fileLines,
	);
	validateLineBounds(targetEdits, fileLines);
	const duplicateNormalization = dropPureInsertDuplicates(
		targetEdits,
		fileLines,
		options.autoDropPureInsertDuplicates === true,
	);
	targetEdits = duplicateNormalization.edits;
	const indentationWarnings = repairReplacementIndentation(targetEdits, fileLines);
	const leading = [...clipboardWarnings, ...duplicateNormalization.warnings, ...indentationWarnings];
	...
	const finish = (result: Materialized, warnings: string[]): ApplyResult => {
		...
		if (
			!parsesCleanly(options.path, result.text, options.syntaxValidator) &&
			parsesCleanly(options.path, text, options.syntaxValidator)
		) {
	...
	if (parsesCleanly(options.path, authoredResult.text, options.syntaxValidator))
		return finish(authoredResult, authoredWarnings);
	...
	const baselineParses = parsesCleanly(options.path, text, options.syntaxValidator);
	if (authored.sparesProposed) {
		try {
			...
			if (parsesCleanly(options.path, sparedResult.text, options.syntaxValidator)) {
