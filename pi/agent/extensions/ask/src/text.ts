...
export function wrapText(text: string, width: number): string[] {
	...
	const lines = text.split("\n").flatMap((paragraph) => wrapParagraphWithIndentation(paragraph, effectiveWidth));

	return lines.length > 0 ? lines : [""];
}

function wrapParagraphWithIndentation(paragraph: string, width: number): string[] {
	if (paragraph.length === 0) {
		return [""];
	}

	const leadingWhitespace = paragraph.match(LEADING_WHITESPACE_PATTERN)?.[0] ?? "";
