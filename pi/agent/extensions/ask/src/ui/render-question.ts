...
function renderStandardOption(lines: string[], row: OptionRowModel, context: QuestionRenderContext) {
...
function renderPreviewQuestion(context: QuestionRenderContext, model: ReturnType<typeof buildQuestionScreenModel>) {
	if (model.mode !== "preview") {
		return;
	}
	...
	if (model.previewLayout === "custom") {
		...
		renderWidePreviewLayout(add, model.rows, theme, width, model.selectedOption);
	} else {
		renderStackedPreviewLayout(add, model.rows, theme, width, model.selectedOption);
	}
...
function renderWidePreviewLayout(
	...
	const rightWidth = Math.max(UI_DIMENSIONS.previewMinRightWidth, width - leftWidth - 2);
...
function renderPreviewOptionList(rows: OptionRowModel[], theme: Theme, width: number): string[] {
	const lines: string[] = [];
	for (const row of rows) {
		pushWrappedText(lines, `${row.index + 1}. ${row.label}`, width, theme, row.color, row.pointer, "  ");
...
function renderOptionDetail(
	...
	pushWrappedText(lines, detail.text, context.width, context.theme, "muted", "     ", "     ");
}
...
function renderEditorWithIndent(args: {
	...
	renderEditorBlock({
		lines,
		editorLines: editor.render(Math.max(UI_DIMENSIONS.editorMinWidth, width - padding)),
		width,
		theme,
		indent,
...
function renderInteractiveCustomOption(lines: string[], row: OptionRowModel, context: QuestionRenderContext) {
	pushWrappedText(
		lines,
		`${row.index + 1}. ${row.prefix}${row.label}`,
		context.width,
	...
	renderEditorWithIndent({
		...
		indent: "     ",
		padding: UI_DIMENSIONS.editorIndentedPadding,
		placeholder: row.detail?.kind === "editor" ? row.detail.placeholder : "Type answer...",
	});
}

function renderOptionDescription(lines: string[], description: string | undefined, width: number, theme: Theme) {
	if (!description) {
		return;
	}
