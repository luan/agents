...
export function pushWrappedText(
	...
	for (let index = 0; index < wrapped.length; index++) {
		...
		lines.push(truncateToWidth(`${currentPrefix}${theme.fg(color, line)}`, width));
	}
	if (wrapped.length === 0) {
...
export function renderEditorBlock(args: {
	...
	if (isEmpty && placeholder) {
		lines.push(
			truncateToWidth(`${indent}${renderInputLine(placeholder, availableWidth, theme, placeholderColor)}`, width),
		);
		return;
	}
...
function isEditorBorderLine(line: string): boolean {
	...
	return EDITOR_BORDER_PATTERN.test(plainText) || EDITOR_SCROLL_BORDER_PATTERN.test(plainText);
}
...
function stripAnsiColorCodes(text: string): string {
	...
	while (true) {
		...
		const end = result.indexOf(ANSI_TERMINATOR, start + ANSI_CONTROL_SEQUENCE.length);
		if (end === -1) {
			return result;
		}

		result = result.slice(0, start) + result.slice(end + ANSI_TERMINATOR.length);
	}
}

function renderEditorLine(line: string, availableWidth: number, theme: Theme): string {
	const innerWidth = Math.max(4, availableWidth - 2);
	const truncated = truncateToWidth(line, innerWidth);
	const padding = " ".repeat(Math.max(0, innerWidth - visibleWidth(truncated)));
	return renderPersistentBackground(`${truncated}${padding}`, theme, "selectedBg");
}
...
export function renderPreviewPaneContent(
	...
	const content: Array<{ text: string; color: ThemeColor }> = [{ text: selectedOption.label, color: "accent" }];
...
export function mergeColumns(left: string[], right: string[], leftWidth: number, width: number): string[] {
	const lines: string[] = [];
...
export function measurePreviewLeftWidth(
	...
	const maxWidth = Math.min(UI_DIMENSIONS.previewLeftMaxWidth, Math.floor(width * UI_DIMENSIONS.previewLeftRatio));
