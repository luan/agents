...
export function moveTab(state: AskState, delta: number): AskState {
	...
	return {
		...
		activeOptionIndex: 0,
		activeSubmitActionIndex: 0,
		view: activeTabIndex >= state.questions.length ? submitView() : navigateView(),
	};
}
...
export function moveOption(state: AskState, delta: number): AskState {
	...
	if (isSubmitTab(state)) {
		return {
			...state,
			activeSubmitActionIndex: clamp(state.activeSubmitActionIndex + normalizedDelta, 0, SUBMIT_ACTION_COUNT - 1),
		};
	}
	...
	return {
		...state,
		activeOptionIndex: clamp(state.activeOptionIndex + normalizedDelta, 0, Math.max(0, options.length - 1)),
	};
}
