...
const ctVaultCreateSchema = Type.Object({
	...
	source: Type.Optional(Type.String({ description: "Source artifact stem for wiki-linking" })),
	tags: Type.Optional(Type.String({ description: "Comma-separated tags" })),
	dive: Type.Optional(Type.Boolean({ description: "Route to dive/ instead of spec/" })),
});

const ctVaultListSchema = Type.Object({
	kind: Type.Optional(vaultKindSchema),
	project: Type.Optional(Type.String({ description: "Filter by project path" })),
	all: Type.Optional(Type.Boolean({ description: "Show artifacts from all projects" })),
	archived: Type.Optional(Type.Boolean({ description: "Show archived artifacts" })),
...
const ctVaultReadSchema = Type.Object({
	...
	frontmatter: Type.Optional(Type.Boolean({ description: "Output frontmatter as JSON" })),
});

const ctVaultArchiveSchema = Type.Object({
	kind: Type.Optional(vaultKindSchema),
	target: Type.Optional(Type.String({ description: "Artifact file path or stem" })),
	targets: Type.Optional(Type.Array(Type.String({ description: "Artifact file path or stem" }))),
...
function formatCommand(command: string, args: string[]): string {
	return [command, ...args.map((arg) => (arg.includes(" ") || arg.includes("\t") ? JSON.stringify(arg) : arg))].join(
		" ",
	);
}
...
async function runCt(args: string[], cwd: string, signal?: AbortSignal, input?: string): Promise<CtResult> {
	return runCommand("ct", args, cwd, signal, input);
}

function toolResult(command: string, cwd: string, result: CtResult, transform?: (text: string) => string) {
	const stdout = trimOutput(result.stdout);
	const stderr = trimOutput(result.stderr);
	const base = transform ? transform(stdout || stderr) : mergeOutput(result.stdout, result.stderr);
	const text = transform && stderr ? `${trimOutput(base)}\n\n[stderr]\n${stderr}` : base;
	return {
...
function renderVaultResult(operation: string, result: any, theme: any, ctx: any) {
	...
	return renderText(
		ctx,
		line(theme, [chip(theme, vaultIcon(operation), operation, summarizeVaultOutput(raw || stderr))]),
	);
}
...
export default function vaultExtension(pi: ExtensionAPI) {
	...
	registerTool({
		...
		promptGuidelines: ["Use create + edit + commit. Never write vault files directly."],
		parameters: ctVaultCreateSchema,
		executionMode: "sequential",
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			const args = ["vault", "create", "-t", params.kind, "--topic", params.topic];
	...
	registerTool({
		...
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			...
			const result = await runCt(args, ctx.cwd, signal);
			return toolResult(formatCommand("ct", args), ctx.cwd, result, useJson ? prettyJson : undefined);
		},
	...
	registerTool({
		...
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			...
			const result = await runCt(args, ctx.cwd, signal);
			return toolResult(formatCommand("ct", args), ctx.cwd, result, params.frontmatter ? prettyJson : undefined);
		},
	...
	registerTool({
		...
		renderCall: (args, theme, ctx) =>
			renderVaultCall("archive", args.target ?? `${args.targets?.length ?? 0} targets`, theme, ctx),
	...
	registerTool({
		...
		renderCall: (args, theme, ctx) =>
			renderVaultCall("prune", `${args.kind ?? "all"} · ${args.days ?? "default"}d`, theme, ctx),
	...
	registerTool({
		...
		promptGuidelines: ["Use when the artifact slug should change; keep wiki-links in mind."],
