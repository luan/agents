...
export function supportsNativeImageGeneration(
  model: ExtensionContext["model"],
): boolean {
  return isOpenAICodexModel(model) && supportsImageInputs(model);
}

function isFunctionToolNamed(
  tool: unknown,
  name: string,
): tool is FunctionToolPayload {
  return (
    !!tool &&
    typeof tool === "object" &&
...
export function rewriteNativeWebSearchTool(
  payload: unknown,
  model: ExtensionContext["model"],
): unknown {
  if (
  ...
  return rewritten
    ? { ...(payload as ResponsesPayload), tools: nextTools }
    : payload;
}

export function rewriteNativeImageGenerationTool(
  payload: unknown,
  model: ExtensionContext["model"],
): unknown {
  if (
...
export function getOpenAICodexLatestImagePath(cwd: string): string {
  return resolve(
    getOpenAICodexImageDirectory(cwd),
    OPENAI_CODEX_LATEST_IMAGE_NAME,
  );
}
...
export async function saveOpenAICodexGeneratedImage(
  ...
  const absolutePath = getOpenAICodexImagePath(
    workspaceRoot,
    image.responseId,
    image.callId,
    outputFormat,
...
export function extractWebSearch(item: unknown): SurfacedWebSearch | undefined {
  ...
  for (const source of Array.isArray(action.sources) ? action.sources : [])
    addSource(source);
  for (const source of Array.isArray(record.results) ? record.results : [])
    addSource(source);
...
export function buildWebSearchActivityMessage(
  searches: SurfacedWebSearch[],
): string {
  return searches
    .map((search, index) => {
      const lines = [
        searches.length > 1
          ? `Web search results ${index + 1}`
          : "Web search results",
      ];
...
export function buildWebSearchSummaryText(
  searches: SurfacedWebSearch[],
): string {
  return searches.length === 1
    ? "Searched the web once"
    : `Searched the web ${searches.length} times`;
}
...
export function createImageGenerationTool(): ToolDefinition<any> {
  return {
    ...
    renderCall(_args, theme) {
      return new Text(
        theme.fg("toolTitle", theme.bold(IMAGE_GENERATION_TOOL_NAME)),
        0,
        0,
      );
    },
...
export function renderImageGenerationMessage(
  ...
  box.addChild(
    new Text(
      theme.fg("customMessageLabel", theme.bold("[image_generation]")),
      0,
      0,
    ),
  );
  if (savedImage) {
    box.addChild(
      new Text(
        `\n${theme.fg("customMessageText", buildGeneratedImageDisplayText(savedImage, options))}`,
        0,
        0,
      ),
    );
...
export function renderWebSearchMessage(
  ...
  box.addChild(
    new Text(
      theme.fg(
        "customMessageLabel",
        theme.bold(buildWebSearchSummaryText(searches)),
      ),
      0,
      0,
    ),
