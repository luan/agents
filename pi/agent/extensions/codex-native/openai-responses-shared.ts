...
function isImageGenerationCallBlock(
  block: InternalAssistantContent,
): block is ImageGenerationCallBlock {
  return (
    block.type === "image_generation_call" &&
    block.item?.type === "image_generation_call"
  );
}

function sanitizeImageGenerationCallItem(
  item: unknown,
...
function replaceImagesWithPlaceholder(
  content: Extract<Message, { role: "user" }> extends { content: infer T }
    ? Exclude<T, string>
    : never,
  placeholder: string,
) {
  ...
  const transformed = imageAwareMessages.map((msg) => {
    ...
    if (msg.role === "assistant") {
      ...
      const transformedContent = (
        assistantMsg.content as InternalAssistantContent[]
      ).flatMap((block) => {
        ...
        if (block.type === "thinking") {
          ...
          return isSameModel
            ? block
            : { type: "text" as const, text: block.thinking };
        }
        if (block.type === "text")
          return isSameModel
            ? block
      ...
      return {
        ...assistantMsg,
        content: transformedContent as Extract<
          Message,
          { role: "assistant" }
        >["content"],
      };
...
export function convertResponsesMessages<TApi extends Api>(
  ...
  const normalizeIdPart = (part: string) => {
    ...
    const normalized =
      sanitized.length > 64 ? sanitized.slice(0, 64) : sanitized;
    ...
    if (msg.role === "user") {
      ...
      for (const block of msg.content as InternalAssistantContent[]) {
        if (isImageGenerationCallBlock(block)) {
          const imageGenerationCall = sanitizeImageGenerationCallItem(
            block.item,
          );
          ...
          let msgId =
            parsedSignature?.id ?? `msg_${msgIndex}_${assistantBlockIndex}`;
...
export async function processResponsesStream<TApi extends Api>(
  ...
  for await (const event of openaiStream) {
    if (event.type === "response.created") {
      ...
      const state = outputStates.get(event.output_index);
      if (state?.kind === "reasoning") {
        const summaryPart = state.summaryParts.get(event.summary_index) ?? {
          text: "",
        };
      ...
      if (state?.kind === "message") {
        const messagePart = state.parts.get(event.content_index) ?? {
      ...
      if (state?.kind === "function_call") {
        ...
        state.block.arguments = parseStreamingJson(
      ...
      if (item.type === "reasoning") {
        ...
        if (!state || state.kind !== "reasoning") {
          const currentBlock: ThinkingBlock = {
            type: "thinking",
            thinking: "",
          };
          output.content.push(currentBlock);
        ...
        state.block.text = item.content
          ...
          .join("");
        state.block.textSignature = encodeTextSignatureV1(
          item.id,
        ...
        const toolCallIndex =
          state?.kind === "function_call" ? state.blockIndex : blockIndex();
        stream.push({
          type: "toolcall_end",
