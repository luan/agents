...
function readJsonObject(
  ...
  try {
    ...
    warnings.push(
      `Ignoring ${filePath}: expected a JSON object at the top level.`,
    );
    return undefined;
  } catch (error) {
...
function readConfigBlock(
  ...
  if (!isRecord(rawConfig)) {
    warnings.push(
      `Ignoring ${settingsPath}${kind === "extension" ? "" : `:${EXTENSION_SETTINGS_KEY}`}: expected an object.`,
    );
    return {};
  }
  ...
  const blockPath =
    kind === "extension"
      ? settingsPath
      : `${settingsPath}:${EXTENSION_SETTINGS_KEY}`;

  resolved.enabled = toBoolean(
    config.enabled,
    `${blockPath}.enabled`,
    warnings,
  );
  ...
  resolved.notifyOnLoad = toBoolean(
    config.notifyOnLoad,
    `${blockPath}.notifyOnLoad`,
    warnings,
  );
  ...
  if (artifactPathValue !== undefined) {
    if (
      typeof artifactPathValue === "string" &&
      artifactPathValue.trim().length > 0
    ) {
      resolved.artifactRoot = resolveConfiguredPath(
        artifactPathValue.trim(),
        path.dirname(settingsPath),
      );
    } else {
      warnings.push(
        `Ignoring ${blockPath}.artifactRoot: expected a non-empty string.`,
      );
    }
  }
  ...
  resolved.supportedApis = toStringArray(
    config.supportedApis,
    `${blockPath}.supportedApis`,
    warnings,
  );
...
function applyEnvOverrides(settings: ExtensionSettings): ExtensionSettings {
  ...
  return {
    ...
    logProviderPayloads:
      resolveEnvBoolean("LOG_PROVIDER_PAYLOADS") ??
      settings.logProviderPayloads,
    logCompactResponses:
      resolveEnvBoolean("LOG_COMPACT_RESPONSES") ??
      settings.logCompactResponses,
    redactSensitiveData:
      resolveEnvBoolean("REDACT_SENSITIVE_DATA") ??
    ...
    supportedProviders:
      resolveEnvCsv("SUPPORTED_PROVIDERS") ?? settings.supportedProviders,
...
export function loadExtensionSettings(cwd?: string): LoadedExtensionSettings {
  ...
  const projectSettingsPath = cwd
    ? path.join(cwd, PROJECT_SETTINGS_DIR, PROJECT_SETTINGS_FILE)
    : undefined;
  ...
  const projectSettings = projectSettingsPath
    ? readJsonObject(projectSettingsPath, warnings)
    : undefined;

  let resolved: ExtensionSettings = { ...DEFAULT_EXTENSION_SETTINGS };

  const extensionConfig = readConfigBlock(
    extensionSettings,
  ...
  const globalConfig = readConfigBlock(
    globalSettings,
  ...
  if (projectSettingsPath) {
    const projectConfig = readConfigBlock(
      projectSettings,
      projectSettingsPath,
      "project",
      warnings,
  ...
  resolved.supportedProviders = [
    ...new Set(
      resolved.supportedProviders.map((item) => item.trim()).filter(Boolean),
    ),
  ];
  resolved.supportedApis = [
    ...new Set(
      resolved.supportedApis.map((item) => item.trim()).filter(Boolean),
    ),
  ];
