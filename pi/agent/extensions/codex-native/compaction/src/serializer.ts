...
export const COMPACTION_SERIALIZER_STRATEGY =
  "local-same-model-responses-serializer" as const;

export type CompactionSerializerStrategy =
  typeof COMPACTION_SERIALIZER_STRATEGY;
...
export function serializeCompactionPreparationToRequest<
  TApi extends Api,
>(args: {
  model: Model<TApi>;
  preparation: CompactionPreparation;
...
export function compareCompactRequestToPayload(
  ...
  if (payload.model !== request.model) {
    mismatches.unshift(
      `model: expected ${payload.model}, got ${request.model}`,
    );
  }
...
function sanitizeImageGenerationCallItem(
  item: unknown,
): ResponsesImageGenerationCallItem | undefined {
