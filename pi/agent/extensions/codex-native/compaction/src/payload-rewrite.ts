...
function isResponsesInputContentItem(
  value: unknown,
...
function cloneOpaqueCompactedWindow(
...
function cloneResponsesInputSlice(
...
function isPromptEnvelopeItem(
  item: unknown,
): item is ResponsesInputMessageItem {
...
export function extractFreshAuthoritativePreamble(
  ...
  return {
    ...
    leadingInput: payload.input
      ...
      .map((item) =>
        cloneResponsesInputMessageItem(item as ResponsesInputMessageItem),
      ),
...
function collectReplayMessages(
...
function createCompactionSummaryAgentMessage(
...
function findEntryIndexByIdBeforeBoundary(
  ...
  const index = entries.findIndex(
    (entry, candidateIndex) =>
      candidateIndex < boundaryIndex && entry.id === entryId,
  );
...
export function findCompactionBoundaryIndex(
  ...
  const boundaryIndex = entries.findIndex(
    (entry) => entry.id === compactionEntryId,
  );
...
export function collectLiveTailMessages(
  entries: readonly SessionEntry[],
): AgentMessage[] {
...
function buildNativeReplaySegmentsInternal<TApi extends Api>(args: {
  ...
  const boundaryIndex = findCompactionBoundaryIndex(
    args.branchEntries,
  ...
  const compactedWindow = cloneOpaqueCompactedWindow(
    args.compactionEntry.details.compactedWindow,
  );
  if (!compactedWindow) {
    return {
      ok: false,
  ...
  const postCompactionTailMessages = collectReplayMessages(
    postCompactionEntries,
  );
  const compactionSummaryMessage = createCompactionSummaryAgentMessage(
    args.compactionEntry,
  );
  const serializedPiHistoryInput = serializeMessagesToResponsesInput(
    args.model,
    [
...
export function rewriteResponsesPayloadWithNativeReplay<
  TApi extends Api,
>(args: {
  model: Model<TApi>;
  payload: ResponsesCompatibleRequestPayload;
