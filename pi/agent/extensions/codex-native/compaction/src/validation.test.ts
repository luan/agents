...
const TEST_CONTEXT_DIR = path.join(
  os.tmpdir(),
  "openai-native-compaction-validation",
);
...
async function loadHookHarness(options: HookHarnessOptions = {}): Promise<{
  ...
  const { default: extension } = await import(
    `./extension-runtime.ts?test=${crypto.randomUUID()}`
  );
  extension({
...
test("manual /compact preserves tool/result ordering + assistant phases and persists the native shim", async () => {
  ...
  expect(
    (result.compaction.details as { compactedWindow: unknown[] })
      .compactedWindow,
  ).toEqual(compactedWindow);
});
