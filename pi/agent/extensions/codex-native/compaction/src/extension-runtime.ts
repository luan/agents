...
function getCompactionIdentityDebugInfo(
...
async function handleSessionBeforeCompact(
  ...
  let request = undefined as
    | ReturnType<typeof serializeMessagesToCompactRequest>
    | undefined;
  if (latestNativeCompaction.ok) {
    const liveTailEntries = branchEntries.slice(
      latestNativeCompaction.index + 1,
    );
...
async function handleBeforeProviderRequest(
  ...
  const latestAnyNativeCompaction =
    resolveLatestNativeCompactionEntry(branchEntries);
  if (!latestAnyNativeCompaction.ok) {
    writeDebugArtifact(
      "provider-request",
      {
        ...
        latestCompactionIdentity: getCompactionIdentityDebugInfo(
          latestAnyNativeCompaction.latestCompaction,
        ),
  ...
  writeDebugArtifact(
    "provider-request",
    {
      ...
      preCompactionKeptItems:
        rewrite.segments.preCompactionKeptWindow.input.length,
