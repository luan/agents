...
import { describeImage } from "../../runtime/describe-image.ts";
...
export function createViewImageTool(): ToolDefinition<ViewImageParameters, ViewImageDetails> {
	return {
		...
		label: "view_image",
		description:
			"View a local image file from the filesystem when visual inspection is needed. Use this for images already available on disk. Text-only models receive a generated description from the configured vision model instead of image pixels.",
		promptGuidelines: [
			"In Code Mode, forward an image_url result with image(result). If the result has description instead, pass it to text(result.description).",
		],
		...
		async execute(_toolCallId, params, signal, _onUpdate, context) {
			const settings = imageSettings.get();
			const fallback = !supportsViewImageInputs(context.model);
			if (fallback && !settings.descriptionFallback) throw new Error(UNSUPPORTED_MESSAGE);
			...
			const description = fallback
				? await describeImage(output, settings.descriptionModel, context, signal)
				: undefined;
			const result = createViewImageResult(input, output, Math.max(0, Math.round(performance.now() - startedAt)));
			return description
				? {
						content: [
							{
								type: "text",
								text: `Image description generated by ${description.model} (the active model did not view the image):\n\n${description.text}`,
							},
						],
