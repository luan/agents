...
function isNativeCompactionRequestMeta(value: unknown): value is NativeCompactionRequestMeta {
	...
	if (compactedKeptWindow !== undefined && typeof compactedKeptWindow !== "boolean") return false;

	return true;
}

function isNativeCompactionIdentity(value: unknown): boolean {
...
export function isNativeCompactionUsage(value: unknown): value is NativeCompactionUsage {
	if (!isRecord(value)) return false;
	return [value.inputTokens, value.cachedInputTokens, value.cacheWriteInputTokens, value.outputTokens].every(
...
export function isNativeCompactionDetails(value: unknown): value is NativeCompactionDetails {
	...
	return (
		...
		isNativeCompactionIdentity(candidate) &&
