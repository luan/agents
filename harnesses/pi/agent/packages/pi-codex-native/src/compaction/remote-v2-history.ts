...
export const DEFAULT_RETAINED_USER_MESSAGE_TOKEN_BUDGET = 64_000;
...
function validMetadata(value: unknown): boolean {
	return (
		value === undefined ||
		value === null ||
		(isRecord(value) && (value.turn_id === undefined || value.turn_id === null || typeof value.turn_id === "string"))
	);
}

/** Accept only the provider-issued opaque compaction item. */
...
function isInternalModelContext(text: string): boolean {
	...
	return /^<codex_internal_context source="([a-z][a-z0-9_]*)">[\s\S]*<\/codex_internal_context>$/.test(trimmed);
}
...
export function isContextualUserText(text: string): boolean {
	return (
...
function retainedRealUserMessage(item: unknown): Record<string, unknown> | undefined {
	...
	const content = item.content.filter((part) => {
		...
		return !isHookPrompt(part.text) && !isContextualUserText(part.text);
	});
...
/** Build the replay window required by remote-v2: recent real user messages plus opaque output. */
export function buildRemoteCompactionV2Window(
	promptInput: readonly unknown[],
	compactionOutput: Record<string, unknown>,
	maxTokens = DEFAULT_RETAINED_USER_MESSAGE_TOKEN_BUDGET,
