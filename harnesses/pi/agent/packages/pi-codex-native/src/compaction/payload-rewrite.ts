...
const AUTO_COMPACT_STOP_MESSAGE =
	"The previous request exceeded the model context window. The conversation was compacted. Continue from the compacted context.";
import type { ResponsesCompatibleRequestPayload } from "./runtime.ts";
import { isContextualUserText } from "./remote-v2-history.ts";
import {
...
function isPromptEnvelopeItem(item: unknown): item is ResponsesInputMessageItem {
	if (!isResponsesInputMessageItem(item)) return false;
	if (isPreambleRole(item.role)) return true;
	if (item.role !== "user") return false;
	if (typeof item.content === "string") return isContextualUserText(item.content);
	return (
		item.content.length > 0 &&
		item.content.every((part) => part.type === "input_text" && isContextualUserText(part.text))
	);
}
