...
function formatLimitUsage(
	...
	const parts = [primary ? `5h ${primary}%` : undefined, secondary ? `weekly ${secondary}%` : undefined].filter(
		Boolean,
	);
...
export function formatCodexUsageLimitError(value: unknown): string | undefined {
	...
	const reset = formatReset(
		envelope.error?.resets_in_seconds ?? asNumber(header(envelope.headers, "X-Codex-Primary-Reset-After-Seconds")),
