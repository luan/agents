...
function parseResetCredit(value: JsonValue | undefined): CodexRateLimitResetCredit | undefined {
...
export function parseCodexRateLimitResetCreditsPayload(
	...
	const credits = Array.isArray(root["credits"]!)
		? root["credits"]!.map(parseResetCredit).filter((credit): credit is CodexRateLimitResetCredit => Boolean(credit))
		: [];
	return { availableCount, credits, raw: payload };
}

function parseCodexRateLimitResetCreditsSummary(value: JsonValue | undefined): CodexRateLimitResetCredits | undefined {
	if (!isRecord(value)) return undefined;
...
function parseRateLimit(value: JsonValue | undefined): {
	primary?: CodexUsageWindow | undefined;
	secondary?: CodexUsageWindow | undefined;
} {
...
export function parseCodexUsagePayload(payload: JsonValue | undefined): CodexUsageSnapshot {
	const root = isRecord(payload) ? payload : {};
	const limits: CodexUsageLimit[] = [];
	const addLimit = (limitId: string, limitName: string | undefined, source: JsonValue | undefined) => {
