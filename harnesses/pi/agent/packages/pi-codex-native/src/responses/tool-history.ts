...
function outputFamily(item: unknown): ToolFamily | undefined {
	...
	if (item["type"] === "tool_search_output" && item["execution"] !== "server" && typeof item["call_id"] === "string")
		return "search";
	return undefined;
}
...
function syntheticOutput(call: Record<string, unknown>, family: ToolFamily, id: string): Record<string, unknown> {
	if (family === "custom") {
