// type-boundary: native helper JSON is validated by parseVoiceHelperEvent and its field validators.
...
export type VoiceHelperCommand =
	...
	| { type: "send_data"; message: JsonValue }
...
export class BoundedJsonlReader {
	...
	private emitLine(): void {
		let line = this.chunks.length === 1 ? this.chunks[0]! : Buffer.concat(this.chunks, this.byteLength);
		this.chunks.length = 0;
		this.byteLength = 0;
...
export function parseVoiceHelperEvent(value: WireValue): VoiceHelperEvent {
	...
	if (event["type"] === "devices" && validDevices(event["inputs"]) && validDevices(event["outputs"]))
		return event as VoiceHelperEvent;
	...
	if (
		event["type"] === "pcm" &&
		validBase64(event["audio"], MAX_PCM_BYTES) &&
		event["sample_rate"] === 24_000 &&
		event["num_channels"] === 1
	)
		return event as VoiceHelperEvent;
...
function validDevices(value: WireValue): value is VoiceDevice[] {
	return (
		Array.isArray(value) &&
		value.length <= MAX_DEVICES &&
		value.every((item) => {
			if (!item || typeof item !== "object") return false;
			const device = item as Record<string, JsonValue | undefined>;
			return (
				boundedString(device["id"], MAX_DEVICE_BYTES) &&
...
function boundedString(value: WireValue, maxBytes: number): value is string {
	return typeof value === "string" && Buffer.byteLength(value) <= maxBytes;
}

function boundedJson(value: WireValue, maxBytes: number): boolean {
...
function validBase64(value: WireValue, maxBytes: number): value is string {
	return (
