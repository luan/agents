...
class RealtimeTranscriptBuffer {
	...
	append(role: TranscriptEntry["role"], transcript: string, startsTurn = false): void {
	...
	takeHistoryBefore(currentUser: TranscriptEntry): string | undefined {
		...
		const history = currentUserIndex < 0 ? [] : this.entries.slice(0, currentUserIndex).filter(({ final }) => final);
	...
	private bound(): void {
		...
		for (const [role, entry] of this.active) if (!this.entries.includes(entry)) this.active.delete(role);
	}
...
export class RealtimeVoiceTurnTracker {
	...
	delegated(input: string, delegationId: string): TrackedRealtimeDelegation | undefined {
		...
		if (this.activeUserTurn) {
			...
			this.unfinishedUserTurns = this.unfinishedUserTurns.filter((turn) => turn !== active);
			this.delegatedUserFinishes += 1;
			return {
	...
	private finishDelegation(delegation: PendingDelegation, currentUser?: TranscriptEntry): RealtimeVoiceTurn {
