...
export class CodexDictationSession {
	...
	constructor(callbacks: CodexDictationCallbacks) {
		...
		this.transcriber = new CodexDictationTranscriber({
			...
			onStatus: (status) => {
				if (status !== "listening") this.callbacks.onStatus(status);
			},
		});
	...
	async start(auth: CodexVoiceAuth, config: VoiceConfig): Promise<void> {
		this.state = "starting";
		...
		try {
			...
			this.callbacks.onStatus("starting microphone");
		} catch (error) {
	...
	finish(): Promise<void> {
		this.finishPromise ??= this.finishRecording();
	...
	private handleHelperEvent(event: VoiceHelperEvent): void {
		...
		if (event.type === "state" && event.state === "listening" && this.state === "starting") {
