// type-boundary: external WebSocket message events are validated by receive before parsing JSON.
type WireValue = unknown;
import type { JsonValue } from "../wire-value.ts";
import { closeWebSocketSilently, connectWebSocket, extractWebSocketError } from "../transport.ts";
...
export class CodexDictationTranscriber {
	...
	async start(auth: CodexVoiceAuth): Promise<void> {
		...
		try {
			...
			this.pendingBytes = 0;
			for (const pcm of pending) this.append(pcm);
	...
	append(pcm: Buffer): void {
		if (this.state === "idle" || this.state === "starting") {
			// Preserve up to the connection deadline (10 seconds of PCM24); longer startup must be retried.
			if (this.pendingBytes + pcm.length > 480_000) {
				this.callbacks.onError(new Error("Dictation connection took too long; please retry the recording"));
				void this.close();
			} else {
	...
	async finish(): Promise<string | undefined> {
		...
		if (this.audioBytes < MIN_AUDIO_BYTES) {
			await this.close();
			throw new Error("No microphone audio was received. Check microphone access and the selected input device.");
		}
		...
		try {
			const transcript = await Promise.race([
				...
				new Promise<never>((_resolve, reject) => {
					timeout = setTimeout(
						() => reject(new Error("Codex dictation transcription timed out")),
						COMPLETION_TIMEOUT_MS,
					);
				}),
			]);
			if (!transcript) throw new Error("No speech was detected. Try dictating again.");
			return transcript;
		} finally {
	...
	private receive(raw: WireValue): void {
		...
		let event: Record<string, JsonValue | undefined>;
		try {
			const value = JSON.parse(raw.data) as JsonValue;
			if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error();
			event = value as Record<string, JsonValue | undefined>;
		} catch {
			this.fail(new Error("Codex dictation emitted an invalid event"));
			return;
		}
...
function buildDictationSessionUpdate(): Record<string, JsonValue | undefined> {
	return {
		type: "session.update",
		session: {
			type: "transcription",
...
function remoteError(event: Record<string, JsonValue | undefined>): string {
	...
	return error &&
		...
		: "Codex dictation error";
}

function closeReason(event: WireValue): string {
	if (!event || typeof event !== "object") return "";
	const value = event as Record<string, JsonValue | undefined>;
