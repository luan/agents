...
export class VoiceHelperClient {
	...
	get protocolVersion(): number | undefined {
		return this.helperProtocolVersion;
	}
	...
	async start(): Promise<void> {
		...
		const binary = await resolveVoiceHelperBinary();
		...
		child.stdin.on("error", (error) => {
			const failure = new Error(`Voice helper write failed: ${stderr || error.message}`);
			ready.reject(failure);
			this.handleStdinError(child, failure);
		});
		const lines = new BoundedJsonlReader(
			MAX_HELPER_LINE_BYTES,
			(line) => {
				try {
					const event = parseVoiceHelperEvent(JSON.parse(line));
					if (event.type === "ready") {
						if (event.version === 2 || event.version === 3 || event.version === 4 || event.version === 5) {
		...
		child.once("error", (error) => {
			const failure = new Error(`Voice helper failed: ${error.message}`);
			ready.reject(failure);
		...
		try {
			await Promise.race([
				...
				new Promise<void>((_resolve, reject) => {
					timeout = setTimeout(
						() => reject(new Error(`Codex voice helper did not become ready within ${READY_TIMEOUT_MS}ms`)),
						READY_TIMEOUT_MS,
					);
				}),
			]);
