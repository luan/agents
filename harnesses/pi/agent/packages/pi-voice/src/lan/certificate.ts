...
export async function resolveLanVoiceCertificate(agentDir: string): Promise<LanVoiceCertificate> {
	const hostnames = [...new Set([hostname(), "localhost"].filter(Boolean))].sort();
	const ipAddresses = lanAddresses();
	const identities = [...hostnames.map((value) => `dns:${value}`), ...ipAddresses.map((value) => `ip:${value}`)].sort();
	...
	const generated = await generate([{ name: "commonName", value: hostnames[0] ?? "localhost" }], {
		algorithm: "sha256",
		notBeforeDate,
...
function certificateIsReusable(certPath: string, keyPath: string, metadataPath: string, identities: string[]): boolean {
	if (!existsSync(certPath) || !existsSync(keyPath) || !existsSync(metadataPath)) return false;
	try {
		const metadata = JSON.parse(readFileSync(metadataPath, "utf8")) as {
			identities?: string[];
		};
		if (!Array.isArray(metadata.identities) || metadata.identities.join("\n") !== identities.join("\n")) return false;
		...
		return (
			...
			Date.parse(certificate.validTo) - Date.now() > RENEW_BEFORE_MS
		);
	} catch {
		return false;
	}
