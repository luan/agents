import type { NestedToolAdapter, RuntimeContentItem, RuntimeResponse } from "../protocol/types.ts";
import { renderCodeModeToolDescription } from "../tools/exec/description.ts";
...
export function toWireToolDefinition(tool: NestedToolAdapter) {
	if (tool.kind === "function" && !tool.parameters)
		throw new Error(`Function code-mode tool requires inputSchema: ${tool.name}`);
	return {
		name: tool.name,
		tool_name: { name: tool.name, namespace: null },
		description: renderCodeModeToolDescription(tool),
		kind: tool.kind,
		input_schema: tool.kind === "freeform" ? null : (tool.parameters ?? null),
...
export function parseExecSource(source: string): {
	...
	if (!trimmed.startsWith("// @exec:")) return { code: source, yieldTimeMs: null, maxOutputTokens: null };
	if (rest.join("\n").trim() === "") throw new Error("exec pragma must be followed by JavaScript source");
	const options = JSON.parse(trimmed.slice("// @exec:".length).trim()) as Record<string, unknown>;
...
function isImageDetail(value: unknown): value is "auto" | "low" | "high" | "original" | null | undefined {
	return (
		value === undefined ||
		value === null ||
		value === "auto" ||
...
function parseInteger(value: unknown, name: string, minimum = 0, maximum = Number.MAX_SAFE_INTEGER): number | null {
...
export function isMissingRuntimeOutcome(value: unknown): boolean {
	return Boolean(isRecord(value) && isRecord(value["outcome"]) && "MissingCell" in value["outcome"]);
}
